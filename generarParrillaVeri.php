...
//$sql_ciudades = mysql_query("SELECT cod_ciudad from ciudades where cod_ciudad in (123)");
echo "<table border=1>";
...
while ($row_cod_ciudad = mysql_fetch_array($sql_ciudades)) {
	...
	$sqlLinea="select l.codigo_linea from lineas l where l.estado=1 and l.linea_promocion=1 and codigo_linea  in (1021)";
	...
	while($datLinea=mysql_fetch_array($respLinea)){
		...
		while($datGrilla=mysql_fetch_array($respGrilla)){
			...
			$sqlNumMed="SELECT COUNT(DISTINCT (rmd.cod_med)) FROM rutero_maestro_cab_aprobado rmc, rutero_maestro_aprobado rm, 
				...
				and rmc.codigo_ciclo = 5 and rmd.categoria_med = '$catMed' and rmd.cod_especialidad = '$codEspe' and 
			...
			if($numeroMed>0){
				for($ii=1; $ii<=$frecuencia; $ii++){
					...
					$sqlInsert="INSERT INTO parrilla (codigo_parrilla,cod_ciclo,cod_especialidad,categoria_med,
						...
						VALUES ($codigoParr, 3, '$codEspe','$catMed',$codigoLinea, '$fechaInsercion', '$fechaInsercion', 
