...
echo "<script language='JavaScript'>
	...
	{	var i, indice;
		...
		var global_linea,global_ciclo,global_gestion;
		global_linea=f.global_linea.value;
		global_ciclo=f.global_ciclo.value;
		global_gestion=f.global_gestion.value;
		...
		ajax=nuevoAjax();
	    ajax.open('POST', 'guardaRegistroDistribucionBanco.php');
   		ajax.onreadystatechange=function()
		{
			...
			if (ajax.readyState==4) {
				...
				location.href='registroDistribucionBanco.php';
			}
		}
		ajax.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded;');
    	ajax.send('vector_campos='+vector_campos+'&global_linea='+global_linea+'&global_ciclo='+global_ciclo+'&global_gestion='+global_gestion);
...
$global_gestion_distribucion = $gestion;
$global_ciclo_distribucion = $ciclo;
...
echo "<input type='hidden' name='global_ciclo' value='$global_ciclo_distribucion'>";
echo "<input type='hidden' name='global_gestion' value='$global_gestion_distribucion'>";
echo "<center><table border='0' class='textotit'><tr><th>Distribuci&oacute;n Banco De Muestras: $global_gestion_distribucion Ciclo: $global_ciclo_distribucion</th></tr></table></center><br>";
...
$sql_productos = "
		...
		from `distribucion_banco_muestras` d where d.cod_ciclo = $global_ciclo_distribucion and d.`codigo_gestion` = '$global_gestion_distribucion' 
...
for ($j = 1; $j <= $indice_productos; $j++) {
    ...
    $sql_territorios = "select cod_ciudad, descripcion from ciudades where cod_ciudad<>'115' order by descripcion";
    $resp_territorios = mysql_query($sql_territorios);
    while ($dat_territorios = mysql_fetch_array($resp_territorios)) { //echo "hola";
        $cod_territorio = $dat_territorios[0];
        $nombre_territorio = $dat_territorios[1];
        $sql_lineaterritorio_producto = "select sum(cantidad_planificada), sum(cantidad_distribuida) from distribucion_banco_muestras
        ...
        if ($cantidad_a_sacar == 0) { //$cadena_txt="<input type='hidden' name='' value='$valor_maximo_asacar'><input type='text' disabled='true' class='texto' value='' name='$cod_territorio|$codigo_producto|' size='3'>";
    ...
    if ($bandera == 1) {
        ...
        /* $sql_cant_sacada="select sum(cantidad_distribuida), sum(cantidad_sacadaalmacen) from distribucion_grupos_especiales where
          ...
          codigo_producto='$codigo_producto'"; */
        $sql_cant_sacada = "select sum(cantidad_distribuida), sum(cantidad_sacadaalmacen) from distribucion_banco_muestras where
        ...
        //CANTIDAD SACADA DE LA DISTRIBUCION ESPECIAL
        
        $sql_cant_sacadaDistEspcial = "select sum(cantidad_distribuida), sum(cantidad_sacadaalmacen) from distribucion_grupos_especiales where
			codigo_gestion='$global_gestion_distribucion' and cod_ciclo='$global_ciclo_distribucion' and
			codigo_producto='$codigo_producto'";
        $resp_cant_sacadaDistEspcial = mysql_query($sql_cant_sacadaDistEspcial);
        $dat_cant_sacadaDistEspcial = mysql_fetch_array($resp_cant_sacadaDistEspcial);
        $cant_sacada_paisEspcial = $dat_cant_sacadaDistEspcial[0];
        $cant_sacada_almacenEspcial= $dat_cant_sacadaDistEspcial[1];
        
        //FIN CANTIDAD SACADA DISTRIBUCION ESPECIAL
        

        $sql_cant_sacadaLinea = "select sum(cantidad_distribuida), sum(cantidad_sacadaalmacen) from distribucion_banco_muestras where
			codigo_gestion='$global_gestion_distribucion' and cod_ciclo='$global_ciclo_distribucion' and
			codigo_producto='$codigo_producto'";
        ...
        $maximo_a_sacarporitem = $maximo_a_sacarporitem - $cant_sacada_paisNormal + $cant_sacada_almacenNormal - $cant_sacada_paisEspcial + $cant_sacada_almacenEspcial;
...
while ($dat_territorios = mysql_fetch_array($resp_territorios)) {
    $cod_territorio = $dat_territorios[0];
    $nombre_territorio = $dat_territorios[1];
    $sql_totales = "select sum(cantidad_planificada), sum(cantidad_distribuida) from distribucion_banco_muestras
...
echo"\n<br><table align='center'><tr><td><a href='navegador_lineas_distribucion.php'><img  border='0'src='imagenes/volver.gif' width='15' height='8'>Volver Atras</a></td></tr></table>";
