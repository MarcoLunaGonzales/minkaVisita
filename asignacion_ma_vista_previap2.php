...
<head>
    ...
    jQuery(document).ready(function($) {
        $("#vista_previa").click(function(){
            ...
            var ciclo_gestion,linea,namee="";
            ciclo_gestion = $("#ciclo").val();
            linea = $("#linea").val();
            $("#linea option:selected").each(function(){
                namee += $(this).attr('namee')+",";
            })
            var url = $(this).attr("rel")+"?ciclo_gestion="+ciclo_gestion+"&linea="+linea+"&namee="+namee;
            var windowName = "popUp1";//$(this).attr("name");
                                ...
                                <td>
                                    ...
                                    <select name="linea" id="linea" multiple size="20">
                                        <?php while($row_linea = mysql_fetch_array($sql_linea)){ ?>
                                            <option value="<?php echo $row_linea[0] ?>" namee="<?php echo $row_linea[1]; ?>"><?php echo $row_linea[1] ?></option>
                                        <?php } ?>
                                    </select>
                                </td>
                            </tr>
                        </table> 
                        <a href="javascript:void(0)" rel="reporte_asignacion_ma2.php" class="button" id="vista_previa">Ir al Resumen</a>
                    </center>
                </div>
            </div>
        </div>
