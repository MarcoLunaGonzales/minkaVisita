...
$numero_contactos=$vector[3];
	...
	//esto es para el codigo de los contactos
		$sql_upd="update contactos set dia_contacto='$dia_contacto', fecha='$fecha_real', turno='$turno' where cod_contacto=$cod_contacto";
	...
	{	$orden_visita=$vector[$i];
		...
		$sql="update contactos_detalle set orden_visita=$orden_visita, cod_med='$cod_med' ,cod_especialidad='$cod_espe', categoria_med='$cod_categoria', cod_zona='$cod_zona' where cod_contacto=$cod_contacto and orden_visita=$orden_visita";
		$resp=mysql_query($sql);
	}
	//sacamos el numero de contactos que se tiene
	echo $cod_contacto;
	$sql_contactos="select * from contactos_detalle where cod_contacto=$cod_contacto";
	...
	$numero_contactos_bd=mysql_num_rows($resp_contactos);
		if($numero_contactos<$numero_contactos_bd)
		{
			...
			{	$sql_del="delete from contactos_detalle where cod_contacto=$cod_contacto and orden_visita=$i";
		...
		{	$entero=intval($i/5);
			...
			{	
				...
				$sql="insert into contactos_detalle values($cod_contacto ,'$i' , '$global_visitador' , '$cod_med' , '$cod_espe' , '$cod_categoria' , '$cod_zona' ,0)";
				$resp=mysql_query($sql);
			}
		}
