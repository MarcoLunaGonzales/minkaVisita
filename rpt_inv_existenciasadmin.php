...
if ($rpt_territorio != 0) {
    ...
    if ($tipo_item == 1) {
        $sql_item = "SELECT codigo, descripcion, presentacion from muestras_medicas $cad order by descripcion, presentacion";
    }
    if ($tipo_item == 2) {
        $sql_item = "SELECT codigo_material, descripcion_material from material_apoyo where codigo_material<>0 $cadena order by descripcion_material";
    }
    // echo $sql_item;
    $resp_item = mysql_query($sql_item);
    if ($tipo_item == 1) {
        echo "<br><table cellspacing='0' border=1 align='center' class='texto'><tr><th>&nbsp;</th><th>C&oacute;digo</th><th>Muestra</th><th>L&iacute;nea</th>
			<th>Cantidad</th><th>Stock Minimo</th><th>Stock Reposicion</th><th>Diferencia</th></tr>";
    }
    if ($tipo_item == 2) {
        if ($imagenes_mostrar == 1) {
            $imagenes_mostrar1 = "<th>Imagen</th>";
        } else {
            $imagenes_mostrar1 = "";
        }
        echo "<br><table cellspacing='0' border=1 align='center' class='texto'><tr><th>&nbsp;</th><th>Material de Apoyo</th><th>Fecha De expiraci&oacute;n</th><th>L&iacute;nea</th><th>Tipo de Material</th><th>Cantidad</th>$imagenes_mostrar1</tr>";
    }
    $indice = 1;
    while ($datos_item = mysql_fetch_array($resp_item)) {
        ...
        if ($tipo_item == 2) {
            ...
            $sql_nombre_linea = "SELECT l.nombre_linea from material_apoyo m, lineas l
                ...
                // echo $sql_nombre_linea."<br />";
            $resp_nombre_linea = mysql_query($sql_nombre_linea);
            $dat_nombre_linea = mysql_fetch_array($resp_nombre_linea);
            $nombre_linea = $dat_nombre_linea[0];

            $sql_tipomaterial = "SELECT t.nombre_tipomaterial,m.url_imagen, m.fecha_expiracion from material_apoyo m, tipos_material t
            ...
            $url_material = $dat_tipomaterial[1];
            $expiracion = $dat_tipomaterial[2];
            if ($url_material == '') {
                $url_material = 'prohibido-queda-1.png';
            }
            if ($imagenes_mostrar == 1) {
                $mostrar = "<td><img src='lib/assets/uploads/$url_material' alt='' style='width:120px' /></td>";
            } else {
                $mostrar = "";
            }
            
            //$cadena_mostrar="<tr><td>$indice</td><td>$nombre_item</td><td>$nombre_linea</td><td>$nombre_tipomaterial</td><td>$stock_real</td></tr>";
            $cadena_mostrar = "<tr><td>$indice</td><td>$nombre_item</td><td>$expiracion</td></td><td>$nombre_linea</td><td>$nombre_tipomaterial</td>";
        }
        ...
        if ($tipo_item == 1) {
            ...
            $cadena_mostrar.="<td align='center'>$stock2</td>$mostrar</tr>";
        }
        ...
        if ($rpt_linea != 0 and $num_filas == 0) { //no se muestra nada
        } else {
            if ($rpt_ver == 1) {
                echo $cadena_mostrar;
            }
...
if ($rpt_territorio == 0) {
    ...
    if ($tipo_item == 2) {
        if ($rpt_tipomaterial == 0) {
            ...
            $sql_item = "SELECT codigo_material, descripcion_material from material_apoyo
				where codigo_material<>0 and cod_tipo_material in ($rpt_tipomaterial) $cadena order by descripcion_material";
        }
    }
    $resp_item = mysql_query($sql_item);
    $indice = 1;
