<script language='JavaScript'>
    ...
    {	var rpt_visitador,rpt_gestion, rpt_ciclo;
        var rpt_territorio;
        var rpt_ver,rpt_ver_dis;
        rpt_ver=f.rpt_ver.value;
        rpt_ver_dis = f.rpt_ver_dis.value;
        ...
        var j=0;
        for(var i=0;i<=f.linea_rpt.options.length-1;i++)
        {	if(f.linea_rpt.options[i].selected)
            {	rpt_linea[j]=f.linea_rpt.options[i].value;
                rpt_nombrelinea[j]=f.linea_rpt.options[i].innerHTML;
                j++;
            }
        }
        j=0;
        ...
        window.open('rpt_central_distribucion.php?rpt_visitador='+rpt_visitador+'&rpt_gestion='+rpt_gestion+'&rpt_ciclo='+rpt_ciclo+'&rpt_territorio='+rpt_territorio+'&rpt_linea='+rpt_linea+'&rptNombreLinea='+rpt_nombrelinea+'&rpt_ver='+rpt_ver+'&rpt_ver_dis='+rpt_ver_dis+'','','scrollbars=yes,status=no,toolbar=no,directories=no,menubar=no,resizable=yes,width=1000,height=800');
        return(true);
    }
</script>
<?php
...
echo "<center><table class='textotit'><tr><th>Reporte Distribución x Ciclo x Visitador</th></tr></table><br>";
...
echo "</select></td></tr>";
echo "<tr><th align='left'>Tipo de Distribucion:</th>";
echo "<td><select name='rpt_ver_dis' class='texto'>";
echo "<option value='0'>Distribucion Normal</option>";
echo "<option value='1'>Distribucion Grupos Especiales</option>";
echo "<option value='2'>Distribucion Banco de muestras</option>";
echo "</select></td></tr>";
