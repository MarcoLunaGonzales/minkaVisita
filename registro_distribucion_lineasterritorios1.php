...
$sql_territorios="SELECT nombre_linea from lineas where codigo_linea='$global_linea'";
...
for($j=1;$j<=$indice_productos;$j++) {  
 ...
 while($dat_territorios=mysql_fetch_array($resp_territorios)) {
    ...
    $sql_lineaterritorio_producto="SELECT sum(cantidad_planificada), sum(cantidad_distribuida), sum(cantidad_devolucion_aplicada_visitador) 
	...
	$cantidad_devoluciones=$dat_lineaterritorio_producto[2];
	
	if($cantidad_devoluciones>0){
		$txtDevoluciones="<br><span style='color:red; font-size:8px;'>($cantidad_devoluciones)</span>";
	}else{
		$txtDevoluciones="";
	}
    ...
    $cadena_producto .= "<td class='celda' bgcolor='#ffffcc' align='right' width='40px' title='$nombre_territorio $nombre_producto'>$cantidad_planificada&nbsp;$txtDevoluciones<input type='hidden' id='celP$j"."_$col' value='$cantidad_planificada'></td>";
...
if($bandera==1) {
    $sql_stock="SELECT SUM(id.cantidad_restante) from ingreso_detalle_almacenes id, ingreso_almacenes i where id.cod_material='$codigo_producto' and i.cod_ingreso_almacen=id.cod_ingreso_almacen and i.ingreso_anulado=0 and i.cod_almacen='1000'";
 ...
 $sql_cant_sacada="SELECT sum(cantidad_distribuida), sum(cantidad_sacadaalmacen) from distribucion_productos_visitadores where codigo_gestion='$global_gestion_distribucion' and cod_ciclo='$global_ciclo_distribucion' and codigo_producto='$codigo_producto'";
 ...
 $sql_cant_sacadaGrupo="SELECT sum(cantidad_distribuida), sum(cantidad_sacadaalmacen) from distribucion_grupos_especiales where codigo_gestion='$global_gestion_distribucion' and cod_ciclo='$global_ciclo_distribucion' and codigo_producto='$codigo_producto'";
 ...
 $sql_cant_sacadaLinea="SELECT sum(cantidad_distribuida), sum(cantidad_sacadaalmacen) from distribucion_productos_visitadores where codigo_gestion='$global_gestion_distribucion' and cod_ciclo='$global_ciclo_distribucion' and codigo_producto='$codigo_producto' and codigo_linea='$global_linea'";
