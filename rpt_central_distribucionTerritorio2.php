...
{   $cod_territorio=$dat_territorios[0];
    ...
    echo "<th colspan='9'>$nombre_territorio</th>";
}	
echo "<th colspan='9'>TOTALES</th>";
...
while($dat_territorios=mysql_fetch_array($resp_territorios))
{
    ...
    echo "<th>CP(GE)</th><th>CD(GE)</th><th>Dif.(GE)</th><th>CP(B)</th><th>CD(B)</th><th>Dif.(B)</th>";
}
echo "<th>CP</th><th>CD</th><th>Dif.</th>";
echo "<th>CP(GE)</th><th>CD(GE)</th><th>Dif.(GE)</th><th>CP(B)</th><th>CD(B)</th><th>Dif.(B)</th>";
echo "</tr>";
...
{   $cod_prod=$dat_productos[0];
    ...
    $resp_territorios=mysql_query($sql_territorio);
    //
    $suma_cantidadfaltante=0; $suma_cantidadfaltante2=0;
    $totalPlanificado=0;      $totalPlanificado2=0;
    $totalDistribuido=0;      $totalDistribuido2=0;
    $totalDistribuido2_b = 0; $totalPlanificado2_b = 0;
    $totalFaltante=0;         $totalFaltante2=0; $totalFaltante2_b = 0; $totalBMM=0;
    //
     while($dat_territorios=mysql_fetch_array($resp_territorios))
    {   $cod_territorio=$dat_territorios[0];
        $nombre_territorio=$dat_territorios[1];
        ...
        $sql_dist="SELECT sum(cantidad_planificada), sum(cantidad_distribuida), sum(cantidad_bancomm)
        ...
        if($cantidad_planificada==""){$cantidad_planificada=0;}
        if($cantidad_distribuida==""){$cantidad_distribuida=0;}
        $cad_mostrar.="<td bgcolor='#CED8F6'>$cantidad_planificada</td>
        ...
        $sql_dist_b = "SELECT sum(cantidad_planificada), sum(cantidad_distribuida)
...
//        echo $sql_dist;
        ...
        $cad_mostrar .="<td bgcolor='#CED8F6'>$cantidad_planificada_b</td>
