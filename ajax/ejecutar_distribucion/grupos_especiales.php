...
$codigo_ciclo = $_REQUEST['cod_ciclo'];
$codigo_gestion = $_REQUEST['cod_gestion'];
/* ---------------------------------------------------------------------------------------------------------------------------------------- */

//$codigo_gestion = $_GET['codGestion'];
//$codigo_ciclo = $_GET['codCiclo'];
//
$global_almacen = 1000;
$tipo_salida = 1000;
$fecha_real = date("Y-m-d");
$hora_salida = date("H:i:s");
$sql_gestion = "select nombre_gestion from gestiones where codigo_gestion='$codigo_gestion'";
$resp_gestion = mysql_query($sql_gestion);
$dat_gestion = mysql_fetch_array($resp_gestion);
$nombre_gestion = $dat_gestion[0];


$observaciones = "SALIDA AUTOMATICA GRUPO ESPECIAL Ciclo: $codigo_ciclo Gesti&oacute;n: $nombre_gestion ";
//aqui inserta la parte de MM

$grupo_salida = 1;
$sql_visitador = "select distinct(f.`codigo_funcionario`), f.`cod_ciudad` from `distribucion_grupos_especiales` d, funcionarios f 
	where f.`codigo_funcionario`=d.`cod_visitador` 
	and d.`codigo_gestion`='$codigo_gestion' and d.`cod_ciclo`='$codigo_ciclo'";
...
//    echo $nro_correlativo;
    ...
    $sql_linea = "select territorio, cod_visitador, codigo_producto, sum(cantidad_planificada), 
			...
			where codigo_gestion='$codigo_gestion' and cod_ciclo='$codigo_ciclo'
...
//    echo "SQL PRODUCTOS:  " . $sql_linea . "<BR><BR>";
    ...
    if ($filas_a_insertar > 0) {
        ...
        $sql_inserta = mysql_query("insert into salida_detalle_visitador values('$codigo','$codvisitador','$codigo_gestion','$codigo_ciclo')");
    }
...
$sqlActDistribucion = "update distribucion_grupos_especiales set cantidad_sacadaalmacen=cantidad_distribuida
			where codigo_gestion='$codigo_gestion' and cod_ciclo='$codigo_ciclo'";
