...
//$codigo_linea = $_REQUEST[ 'ejecutar_distribucion' ];
$global_almacen = 1000;
$tipo_salida = 1000;
...
$observaciones = "SALIDA AUTOMATICA Ciclo: $global_ciclo_distribucion Gesti&oacute;n: $nombre_gestion";
//aqui inserta la parte de MM

$grupo_salida = 1;
$sql_visitador = "select distinct(f.codigo_funcionario), f.cod_ciudad from funcionarios f, funcionarios_lineas fl
				where f.codigo_funcionario=fl.codigo_funcionario and f.cod_cargo='1011' and f.estado=1
...
while ( $dat_visitador = mysql_fetch_array( $resp_visitador ) ) {
	...
	$sql = "select max(nro_correlativo) as nro_correlativo from salida_almacenes where cod_almacen='$global_almacen' and grupo_salida='$grupo_salida'";
	$resp = mysql_query( $sql );
	$dat = mysql_fetch_array( $resp );
	$num_filas = mysql_num_rows( $resp );
	if ( $num_filas == 0 ) {
...
//	echo $nro_correlativo;
	...
	$sql_linea = "select territorio, cod_visitador, codigo_producto, sum(cantidad_planificada), sum(cantidad_distribuida), sum(cantidad_sacadaalmacen), grupo_salida
...
//
	...
	$sql_actualizadistribucion = "update distribucion_productos_visitadores set cantidad_sacadaalmacen=cantidad_distribuida
			 	where codigo_gestion='$global_gestion_distribucion' and cod_ciclo='$global_ciclo_distribucion'
			 	and cod_visitador='$codvisitador' and grupo_salida=1";
	$resp_actualizadistribucion = mysql_query( $sql_actualizadistribucion );
}
...
//	echo $nro_correlativo;
	...
	$sql_actualizadistribucion = "update distribucion_productos_visitadores set cantidad_sacadaalmacen=cantidad_distribuida
			 	where codigo_gestion='$global_gestion_distribucion' and cod_ciclo='$global_ciclo_distribucion'
			 	and cod_visitador='$codvisitador' and grupo_salida=2";
	$resp_actualizadistribucion = mysql_query( $sql_actualizadistribucion );
	
}
echo json_encode( $codigo_linea );
/**/
?>