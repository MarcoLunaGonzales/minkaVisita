...
mysql_query("DELETE from parrillas_automatica");

$sql_veri     = mysql_query("SELECT * from parrilla where cod_ciclo = $ciclo_final and codigo_gestion = $gestion_final");
$sql_num_veri = mysql_num_rows($sql_veri);

if($sql_num_veri >=1){
	$cadena = "";
	$arr = array("cadena" => $cadena,"mensaje" => 'La parrilla ya existe. Si desea generar una nueva, para este ciclo y gestion,  elimine la antigua por favor', "estado" => 1);
			...
			foreach ($cat as $key) {
				...
				for($count = 1; $count <= $cantidad_frecuencia; $count++){
					...
					while ($row_asignacion_productos_grupos = mysql_fetch_array($sql_asignacion_productos_grupos)) {
						...
						if(mysql_num_rows($sql_asignacion_mm) > 0){
							$cantidad = mysql_result($sql_asignacion_mm, 0, 0);
						}else{
							// echo("SELECT ad.cantidad FROM asignacion_mm_excel a, asignacion_mm_excel_detalle ad where a.id = ad.id and a.ciclo = $ciclo_final and a.gestion = $gestion_final and CONCAT_WS(' ',ad.especialidad,ad.linea) = '$especialidad_linea' and producto = '$producto' and ad.categoria = '$key'");
						}

						if($posicion == 1){
							$cadena .= '<tr>'; $cadena .= '<th>Region</th>'; $cadena .= '<th>Especialidad</th>'; $cadena .= '<th>Linea</th>'; $cadena .= '<th>Cat</th>'; $cadena .= '<th>Visita</th>'; $cadena .= '<th>Pos</th>'; $cadena .= '<th>Codigo</th>'; $cadena .= '<th>Producto</th>'; $cadena .= '<th>Cantidad</th>'; /*$cadena .= '<th>Ciudades MA</th>';*/ $cadena .= '<th>Cod MA</th>'; $cadena .= '<th>MA</th>'; $cadena .= '<th>Cantidad</th>'; $cadena .= '</tr>';
						}
						$cadena .= '<tr>';
						...
						// $sql_codigo_l_visita2 = mysql_query("SELECT codigo_l_visita from lineas_visita_nom_generio2 where nom_generico = '$especialidad_linea'");
						// $codigo_l_visita2 = mysql_result($sql_codigo_l_visita2, 0, 0);

						$sql_asignacion_ma = mysql_query("SELECT DISTINCT ad.id_asignacion_ma, ad.codigo_ma, ad.dist_a, ad.dist_b, ad.dist_c , ap.posicion, ad.va_material FROM asignacion_ma_excel a, asignacion_ma_excel_detalle ad, asignacion_ma_excel_posiciones ap where a.id = ad.id_asignacion_ma and ap.id = ad.id and a.ciclo = $ciclo_final and a.gestion = $gestion_final and ad.codigo_l_visita = $codigo_l_visita and a.codigo_mm = '$producto' and ap.posicion = $count and ad.espe_linea = '$especialidad_final $linea_final'  ");
						$cantidad_ma = 0;
						$num_rows = mysql_num_rows($sql_asignacion_ma);
						if($num_rows == ''){
							$codigo_ma = '0';
						}else{
							/*AQUI LA PARTE DE CIUDADES A LAS QUE VA LA MUESTRA*/
							$ciu_fin = '';
							$sql_asignacion_ma_ciudades = mysql_query("SELECT ad.ciudades_pl FROM asignacion_ma_excel a, asignacion_ma_excel_detalle ad where a.id = ad.id_asignacion_ma and a.ciclo = $ciclo_final and a.gestion = $gestion_final and ad.codigo_l_visita = $codigo_l_visita and a.codigo_mm = '$producto' and ad.espe_linea = '$especialidad_final $linea_final'");
							while ($rr = mysql_fetch_array($sql_asignacion_ma_ciudades)) {
								$ciu_fin .= $rr[0].",";
							}
							$ciu_fin = substr($ciu_fin, 0, -1);
							$ciu_fin = explode(",", $ciu_fin);
							if(in_array($row_cod_ciudad[0], $ciu_fin)){
								$va_muestra = mysql_result($sql_asignacion_ma, 0, 6);
	...
	$sql_linea = mysql_query("SELECT codigo_linea from lineas where linea_promocion = 1 and estado = 1");
	...
	// echo $cadena;
	$arr = array("cadena" => $cadena, "mensaje" => 'Guardado Satisfactoriamente', "estado" => 0);
	echo json_encode($arr);
