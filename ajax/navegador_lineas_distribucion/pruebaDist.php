...
$global_linea        = 1021;
$gestionDistribucion = 1010;
$cicloDistribucion   = 10;
...
if ($num_filas_verificacion == 0) {
    ...
    while ($dat_visitador = mysql_fetch_array($resp_visitador)) {
		...
		while ($datRutero = mysql_fetch_array($respRutero)) {
            ...
            $lineaVisita = 0;
            $sqlverificaLineas = "select l.codigo_l_visita from lineas_visita l, lineas_visita_especialidad le, lineas_visita_visitadores_copy lv where l.codigo_l_visita=le.codigo_l_visita and l.codigo_l_visita=lv.codigo_l_visita and le.codigo_l_visita=lv.codigo_l_visita and lv.codigo_funcionario='$codVisitador'and le.cod_especialidad='$codEspe' and lv.codigo_ciclo = $global_ciclo_distribucion and lv.codigo_gestion = 1010 and lv.codigo_linea_visita=$global_linea";
			...
			p.codigo_l_visita = '$lineaVisita' and p.agencia = '$codTerritorio' 
			and pd.codigo_muestra='M109'
		...
		while($datMuestrasQ=mysql_fetch_array($respMuestrasQuit)){
			...
			and lv.codigo_gestion = 1010 and lv.codigo_linea_visita=$global_linea";			
			$respVerificaLineas = mysql_query($sqlverificaLineas);
            $numVerificaLineas = mysql_num_rows($respVerificaLineas);
            if ($numVerificaLineas > 0) {
                $datVerificaLineas = mysql_fetch_array($respVerificaLineas);
