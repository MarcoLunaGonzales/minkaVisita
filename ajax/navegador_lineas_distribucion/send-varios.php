...
$lineas = array(1031, 1032, 1034, 1035);

foreach ($lineas as $global_linea ) {
    ...
    if ($num_filas_verificacion == 0) {
        ...
        while ($dat_visitador = mysql_fetch_array($resp_visitador)) {
            ...
            $sqlRutero = "select count(DISTINCT(rmd.cod_med)), rmd.cod_especialidad, rmd.categoria_med from rutero_maestro_cab_aprobado rmc, rutero_maestro_aprobado rm, rutero_maestro_detalle_aprobado rmd, medicos m where 
            rmc.cod_rutero=rm.cod_rutero and rm.cod_contacto=rmd.cod_contacto and m.cod_med=rmd.cod_med and rmc.estado_aprobado='1' and rmc.codigo_linea='$global_linea' and rmc.codigo_ciclo='$cicloDistribucion' 
            and rmc.codigo_gestion='$gestionDistribucion' and rmc.cod_visitador='$codVisitador' group by rmd.cod_especialidad, rmd.categoria_med";
            ...
            while ($datRutero = mysql_fetch_array($respRutero)) {
                ...
                $sqlParrilla = "select pd.codigo_muestra, (sum(pd.cantidad_muestra)*$cantContactos)
                ...
                and p.agencia='$codTerritorio' group BY(pd.codigo_muestra)";
                ...
                $sqlParrilla = "select pd.codigo_material, (sum(pd.cantidad_material)*$cantContactos)
                from parrilla p, parrilla_detalle pd where p.codigo_parrilla=pd.codigo_parrilla and
                p.codigo_linea='$global_linea' and p.cod_ciclo='$cicloDistribucion'
                and p.codigo_gestion='$gestionDistribucion'
                and p.cod_especialidad='$codEspe' and p.categoria_med='$codCat' and p.codigo_l_visita='$lineaVisita'
                and p.agencia='$codTerritorio' group BY(pd.codigo_material)";
                $respParrilla = mysql_query($sqlParrilla);
                $numFilasParrilla = mysql_num_rows($respParrilla);
