...
$sqlDetalleCodigos="SELECT codigo_material, SUM(cantidad_ing_almacen) FROM devoluciones_ciclodetalle WHERE codigo_devolucion in ($codigos_finales) and cantidad_ing_almacen > 0  GROUP BY 1 ORDER BY 1";
//echo $sqlDetalleCodigos;
$sql_detalle_codigos_materiales = mysql_query($sqlDetalleCodigos);
...
foreach ($codigos_finales_verificacion_stock as $a => $b) {
    $sql_nombre_linea = "SELECT l.nombre_linea, m.stock_minimo, m.stock_reposicion FROM muestras_medicas m, lineas l WHERE m.codigo_linea=l.codigo_linea and m.codigo='$b[0]'";
    ...
    $sql_ingresos = "SELECT sum(id.cantidad_unitaria) FROM ingreso_almacenes i, ingreso_detalle_almacenes id WHERE i.cod_ingreso_almacen=id.cod_ingreso_almacen and i.fecha<='$rpt_fecha' and i.cod_almacen='$almacen_final'and id.cod_material='$b[0]' and i.ingreso_anulado=0 and i.grupo_ingreso='1'";
    $resp_ingresos = mysql_query($sql_ingresos);
    $dat_ingresos = mysql_fetch_array($resp_ingresos);
    $cant_ingresos = $dat_ingresos[0];
    $sql_salidas = "SELECT sum(sd.cantidad_unitaria) FROM salida_almacenes s, salida_detalle_almacenes sd WHERE s.cod_salida_almacenes=sd.cod_salida_almacen and s.fecha<='$rpt_fecha' and s.cod_almacen='$almacen_final'and sd.cod_material='$b[0]' and s.salida_anulada=0 and s.grupo_salida='1'";
...
if ($flag == 0) {
    ...
    $territorio = 115;
    $almacen = 1000;
    $observaciones = "Devolucion de muestras medicas Ciclo: $cod_ciclos | Gestion: $cod_gestiones a almacen central";
    ...
    $sql2 = "SELECT max(nro_correlativo) FROM salida_almacenes WHERE cod_almacen='$global_almacen' and grupo_salida='$grupo_salida' order by cod_salida_almacenes desc";
