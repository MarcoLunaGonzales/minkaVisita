...
//
$codigo_almacenarreglar=1011; //codigo del almacen a corregir, tabla: almacenes
//$cadCodigoMaterial="aa,bb,cc"; //vector de codigos de 'muestra_medica' o 'material_apoyo', a corregir

$cadCodigoMaterial="'M102','M67','M105','M93','M19','M30','CFMM0002','M78','M112','CFCO0166','CFMM0023','M129','CFMM0042','CFCO0121','CFCO0126','M97','M132','M114','M118','CFMM0107','CFMM0018','M115','M117','CFMM0088','M29','M46','M101','M69','M58','M59','CFC00154','M135','CFMM0098','M124','M95','M134','CFMM0108','CFCO0123','M138','M139','CFC00190','CFC00191','CFC00192','CFC00193'";
//
$vecCodigoMaterial=explode(",", $cadCodigoMaterial);
$total=count($vecCodigoMaterial);
echo "<br>";
echo "TOTAL ELEMENTOS A COREGIR: $total<br>";
//
for($i=0;$i<$total;$i++)
{
    //
    ...
    $sql_det_ingreso="
        SELECT id.cod_ingreso_almacen, id.cod_material, id.nro_lote, id.cantidad_unitaria
        FROM ingreso_detalle_almacenes id, ingreso_almacenes i
        WHERE i.cod_ingreso_almacen=id.cod_ingreso_almacen AND i.cod_almacen='$codigo_almacenarreglar' AND i.grupo_ingreso='1'
        AND id.cod_material='$codigo_material'
        ORDER BY cod_ingreso_almacen";
	//echo "<br>$sql_det_ingreso<br>";
    ...
    {   $cod_ingreso=$dat_det_ingreso[0];
        ...
        WHERE cod_ingreso_almacen='$cod_ingreso' AND cod_material='$cod_material' AND nro_lote='$nro_lote'");
    }
    //borramos la tabla salida_detalle_ingreso
    $sql_salida_detalle="
        SELECT sd.cod_salida_almacen, sd.cod_material, sd.cantidad_unitaria, s.grupo_salida
        FROM salida_detalle_almacenes sd, salida_almacenes s
        WHERE s.cod_salida_almacenes=sd.cod_salida_almacen AND s.cod_almacen='$codigo_almacenarreglar' AND s.grupo_salida=1
        AND sd.cod_material='$codigo_material'";
    ...
    $sql_salida_detalle="
        SELECT sd.cod_salida_almacen, sd.cod_material, sd.cantidad_unitaria, s.grupo_salida
        FROM salida_detalle_almacenes sd, salida_almacenes s
        WHERE s.cod_salida_almacenes=sd.cod_salida_almacen AND s.cod_almacen='$codigo_almacenarreglar' AND s.grupo_salida=1 AND s.salida_anulada=0
        and sd.cod_material='$codigo_material'";
    $resp_salida_detalle=mysql_query($sql_salida_detalle);
    while($dat_salida_detalle=mysql_fetch_array($resp_salida_detalle))
    {   $cod_salida_almacen=$dat_salida_detalle[0];
        ...
        $sql_detalle_ingreso="
            ...
            FROM ingreso_detalle_almacenes id, ingreso_almacenes i
            WHERE i.cod_ingreso_almacen=id.cod_ingreso_almacen AND i.cod_almacen='$codigo_almacenarreglar'
            AND id.cod_material='$cod_material' AND id.cantidad_restante<>0 AND i.ingreso_anulado=0
        ...
        {   $cod_ingreso_almacen=$dat_detalle_ingreso[0];
            ...
            {   if($cantidad_bandera>$cantidad_restante)
                {   $sql_salida_det_ingreso="INSERT INTO salida_detalle_ingreso VALUES('$cod_salida_almacen','$cod_ingreso_almacen','$cod_material','','$cantidad_restante','2')";
                ...
                {   $sql_salida_det_ingreso="INSERT INTO salida_detalle_ingreso VALUES('$cod_salida_almacen','$cod_ingreso_almacen','$cod_material','','$cantidad_bandera','2')";
                    $resp_salida_det_ingreso=mysql_query($sql_salida_det_ingreso);
    ...
    //
    echo "Material $codigo_material, corregido.<br>";
    //
}
//fin
echo "OK MUESTRAS.....";
