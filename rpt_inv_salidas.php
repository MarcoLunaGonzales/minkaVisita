...
$nombre_tiposalida=$datos_tipo_salida[0];
	if($tipo_item==1)
	{$nombre_item="Muestra M&eacute;dica";}else{$nombre_item="Material de Apoyo";}
	if($tipo_salida!="")
...
if($tipo_reporte==0)
{
	...
	and c.cod_ciudad=s.territorio_destino and a.cod_almacen=s.cod_almacen and s.salida_anulada=0 and 
	s.grupo_salida='$tipo_item' and s.fecha>='$fecha_iniconsulta' and s.fecha<='$fecha_finconsulta' 
	and s.tipo_salida='$tipo_salida' order by s.nro_correlativo";
	
	if($tipo_salida=="")
	{	$sql="select s.cod_salida_almacenes, s.fecha, ts.nombre_tiposalida, c.descripcion, a.nombre_almacen, 
...
if($tipo_reporte==1)
{	
//                echo "global almacen global_almacen"; 
	if($tipo_item==1)
	...
	{	$codigo_producto=$dat_producto[0];
		...
		$resp=mysql_query($sql);
		$cadena_ciudades="<table border=1 cellspacing='0' class='textomini' width='100%'><tr><th>Territorio Destino</th><th>Cantidad</th></tr>";
		$bandera_producto=0;
		...
		{	$cod_ciudad=$dat[0];
			...
			$sql_salidas="select sum(sd.cantidad_unitaria), s.cod_salida_almacenes from salida_almacenes s, 
				salida_detalle_almacenes sd where sd.cod_salida_almacen = s.cod_salida_almacenes and 
				sd.cod_material = '$codigo_producto' and s.salida_anulada =0 
				and s.territorio_destino='$cod_ciudad' and
				s.cod_almacen = '$global_almacen' and s.fecha >= '$fecha_iniconsulta' and s.fecha <= '$fecha_finconsulta'";
