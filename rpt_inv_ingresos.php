...
if($tipo_item==1) {
	$nombre_item = "Muestra Médica";
}else{
	$nombre_item = "Material de Apoyo";
}
echo "<table align='center' class='textotit'><tr><td align='center'>Reporte Ingresos Almacen<br>$nombre_tipoingresomostrar Fecha inicio: <strong>$fecha_ini</strong> Fecha final: <strong>$fecha_fin</strong> Tipo de Item: <strong>$nombre_item</strong><br>$txt_reporte</td></tr></table>";

$fecha_iniconsulta = cambia_formatofecha($fecha_ini);
$fecha_finconsulta = cambia_formatofecha($fecha_fin);
$sql = "SELECT i.cod_ingreso_almacen, i.fecha, ti.nombre_tipoingreso, i.observaciones, i.nota_entrega, i.nro_correlativo, i.ingreso_anulado, i.cod_orden_compra 
FROM ingreso_almacenes i, tipos_ingreso ti where i.cod_tipoingreso=ti.cod_tipoingreso and i.cod_almacen='$rpt_almacen' and i.grupo_ingreso=$tipo_item and 
i.fecha>='$fecha_iniconsulta' and i.fecha<='$fecha_finconsulta' and i.cod_tipoingreso='$tipo_ingreso' order by i.nro_correlativo";
if($tipo_ingreso=='') {	
	$sql = "SELECT i.cod_ingreso_almacen, i.fecha, ti.nombre_tipoingreso, i.observaciones, i.nota_entrega, i.nro_correlativo, i.ingreso_anulado, i.cod_orden_compra 
	FROM ingreso_almacenes i, tipos_ingreso ti where i.cod_tipoingreso=ti.cod_tipoingreso and i.cod_almacen='$rpt_almacen' and i.grupo_ingreso=$tipo_item and i.fecha>='$fecha_iniconsulta' and i.fecha<='$fecha_finconsulta' order by i.nro_correlativo";
}
...
echo "<tr class='textomini'><th>Nro.</th><th>Nota de Entrega</th><th>Fecha</th><th>OC</th>
<th>Tipo de Ingreso</th><th>Observaciones</th><th>Estado</th><th>&nbsp;</th></tr>";
while($dat=mysql_fetch_array($resp)) {
	...
	$bandera=0;
	$sql_verifica_movimiento="SELECT s.cod_salida_almacenes from salida_almacenes s, salida_detalle_ingreso sdi where s.cod_salida_almacenes=sdi.cod_salida_almacen and s.salida_anulada=0 and sdi.cod_ingreso_almacen='$codigo'";
	...
	$sql_detalle="SELECT i.cod_material, i.nro_lote,i.fecha_vencimiento, i.cantidad_unitaria from ingreso_detalle_almacenes i where i.cod_ingreso_almacen='$codigo'";
	$resp_detalle=mysql_query($sql_detalle);
	$bandera=0;
	$detalle_ingreso="";
	$detalle_ingreso.="<table border=1 cellspacing='0' align='center' class='textomini' width='100%'>";
	$detalle_ingreso.="<tr><th width='60%'>Material</th><th width='10%'>Número de Lote</th><th width='20%'>Fecha Vencimiento</th><th width='10%'>Cantidad</th></tr>";
	while($dat_detalle=mysql_fetch_array($resp_detalle)) {	
		...
		if($rpt_linea!=0 and $num_filas!=0) {	
			$detalle_ingreso.="<tr><td>$nombre_material $presentacion</td><td align='center'>$numero_lote</td>
			<td align='center'>$fecha_vencimiento_mostrar</td>
	...
	if($rpt_linea==0) {	
		echo "<tr bgcolor='$color_fondo'><td align='center'>$nro_correlativo</td><td align='center'>&nbsp;$nota_entrega</td>
		<td align='center'>$fecha_ingreso_mostrar</td>		<td align='center'>$OC</td>

		<td>$nombre_tipoingreso</td><td>&nbsp;$obs_ingreso</td><td>&nbsp;$estado_ingreso</td><td align='center'>$detalle_ingreso</td></tr>";
	}
