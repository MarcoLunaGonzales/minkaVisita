...
$sql_cod_med_categorizacion = mysql_query(" select * from categorizacion_medico ");
$count = 1;
while ($row_c_m_c = mysql_fetch_array($sql_cod_med_categorizacion)) {
    ...
    $nivel = $row_c_m_c[6];
    $sql_especialidad = mysql_query(" select cod_especialidad from categorias_lineas where cod_med = $cod_med_cat ");
...
//        and rc.codigo_gestion = 1009 and rc.estado_aprobado = 1 and rd.cod_med = m.cod_med and m.cod_med = $cod_med_cat ORDER BY 1");
    while ($row_especialidad = mysql_fetch_array($sql_especialidad)) {
        $especialidad = $row_especialidad[0];
...
//        mysql_query(" update medicos set categorizacion_medico = '$categoria_medico_sistea_final' where cod_med = $cod_med_cat ");
        ...
        echo $count . " - " . $cod_med_cat . " " . $categoria_medico_sistea_final . " - " . $ponderacion_especialidad_final . " - " . $ponderacion_edad . " - " . $ponderacion_pacientes . " - " . $ponderacion_prescrptiva . " - " . $ponderacion_nivel . "<br />";
        $count++;
    }
}
?>