...
{	if($medico=="" && $obj=="" && $filtrado=="")
	{	 $sql="select distinct m.cod_med,m.ap_pat_med,m.ap_mat_med,m.nom_med
		 ...
		 where m.cod_ciudad='$rpt_territorio' and m.cod_med=c.cod_med and (p.cod_med=m.cod_med or n.cod_med=m.cod_med) and c.codigo_linea=$global_linea order by m.ap_pat_med";
	}
	else
	{	$sql="select distinct m.cod_med,m.ap_pat_med,m.ap_mat_med,m.nom_med
		 from medicos m, categorias_lineas c
		 where m.cod_ciudad='$rpt_territorio' and m.cod_med=c.cod_med and m.cod_med='$medico' and c.codigo_linea=$global_linea order by m.ap_pat_med";
	}
	...
	while($dat=mysql_fetch_array($resp))
	{
		...
		echo "<tr><td align='center'>$indice_tabla</td><td align='center'>$cod</td><td align='center' class='texto'>$nombre_completo</td><td align='center'>&nbsp;$especialidad</td><td align='center'>$cadena_prod_objetivo</td><td align='center'>$cadena_prod_filtrado</td></tr>";
		$indice_tabla++;
