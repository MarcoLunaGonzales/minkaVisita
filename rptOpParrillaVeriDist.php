...
{	var rpt_visitador,rpt_gestion, rpt_ciclo;
	...
	var rpt_nombrevisitador=new Array();
	var j=0;
	...
	{	if(f.rpt_visitador.options[i].selected)
		{	rpt_visitador[j]=f.rpt_visitador.options[i].value;
			rpt_nombrevisitador[j]=f.rpt_visitador.options[i].innerHTML;
			j++;
		}
	}
	window.open('rptParrillaVeriDist1.php?rpt_visitador='+rpt_visitador+'&rpt_gestion='+rpt_gestion+'&rpt_ciclo='+rpt_ciclo+'&rpt_territorio='+rpt_territorio+'&rpt_linea='+rpt_linea+'&rptNombreLinea='+rpt_nombrelinea+'&rptNombreVisitador='+rpt_nombrevisitador+'','','scrollbars=yes,status=no,toolbar=no,directories=no,menubar=no,resizable=yes,width=1000,height=800');
	return(true);
}
</script>
<?php
...
echo "<center><table class='textotit'><tr><th>Parrillas para Verificacion de Distribucion</th></tr></table><br>";
