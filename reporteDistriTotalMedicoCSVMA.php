...
header("Content-disposition: attachment; filename=\"PLANIMATERIALCICLOMA.csv\""); 
...
while ($datLineas = mysql_fetch_array($respLineas)) {
    ...
    while ($datVis = mysql_fetch_array($respVis)) {
		...
		while ($datRutero = mysql_fetch_array($respRutero)) {
			...
			$sqlParrilla="select p.cod_ma, sum(p.cantidad_ma), m.descripcion_material from parrilla_personalizada p, 
				material_apoyo m where p.cod_linea='$codLinea' and  
			 p.cod_gestion='$codGestion' and p.cod_ciclo='$codCiclo' and p.cod_med='$codMed' 
			 and p.cod_ma=m.codigo_material and p.cod_ma<>0 group by cod_ma, descripcion_material";
