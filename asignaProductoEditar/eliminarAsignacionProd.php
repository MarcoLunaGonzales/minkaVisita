...
function ajaxAgencias(codigo, codLinea){
	...
	ajax=nuevoAjax();
	ajax.open('GET', 'ajaxAgencias.php?codProducto='+codProducto+'&codLineaMkt='+codLinea+'',true);
...
function ajaxEspecialidades(codigo, codLinea){
	...
	ajax=nuevoAjax();
	ajax.open('GET', 'ajaxEspecialidades.php?codProducto='+codProducto+'&codLineaMkt='+codLinea+'',true);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4) {
			contenedor.innerHTML = ajax.responseText;
		}
...
<body>
	<div id="wrapper">
		<h2 align="center">Eliminar Producto Linea: <?php echo $nombreLinea;?>
			<?php echo $espe; ?> <?php echo $contacto; ?> <?php echo $nombreCiudad;?> 
		</h2>

		<div id="form">
			<form id="formulario" method="get" action="guardaEliminarAsignacionProd.php" name="formulario">
			<table border="1" cellpadding="0" cellspacing="0" align="center" width="40%">
				<?php 
				$sqlProd="select distinct(m.codigo), concat(m.descripcion, ' ', m.presentacion) from asignacion_productos_excel_detalle a, 
					muestras_medicas m  where a.linea_mkt=$codLineaMkt 
					and m.codigo=a.producto order By 2;";
			...
			<center><input type="button" value="Eliminar" onClick="envia_formulario(formulario);" name="boton"></center>
		</form>			
		</div>
