                ...
                foreach ($territorios_finales as $valores) { 
                    ...
                    foreach ($codigo_funcionario_final as $val_fun) {
                        ...
                        <div class="four columns end foreach">
                            ...
                            <table border="1" style="height: 600px" class="primerostres">
                                <thead>
                                    <tr>
                                        <th>Producto</th>
                                        <!-- <th>Cantidad</th> -->
                                ...
                                <tbody>
                                    <?php
                                    $sql_productos = mysql_query("SELECT pd.producto, CONCAT(m.descripcion,' ',m.presentacion), pd.contactos from po_p1 p, po_p1_detalle pd, muestras_medicas m where p.id = pd.id and m.codigo = pd.producto and p.ciudad = $valores and p.ciclo = $codigo_ciclo and p.gestion = $codigo_gestion2 and pd.visitador = $val_fun order by 3 DESC");
                                    $num = mysql_num_rows($sql_productos);
                                    $count = 1;

                                    /*Aqui va lo de e array*/
                                    
                                    /*FIN*/
                                    if($num > 0){
                                        while ($row_producto = mysql_fetch_array($sql_productos)) {

                                            // if (array_key_exists($row_producto[0], array_slice($productos_guardados_array,0,5))) {
                                            if (array_key_exists($row_producto[0], $newArray)) {
                                                $newArray[$row_producto[0]] = $newArray[$row_producto[0]] + 1;
                                            }else{
                                                $newArray[$row_producto[0]] =  $cantidad_prodcutos_total_ciudad;
                                            }
                                            // if($newArray[$row_producto[0]] < $canti){
                                                ?>
                                                <tr>
                                                    ...
                                                    <td align="center" style="text-align: center; font-weight: bold"><?php echo $row_producto[2]; ?> <input type="hidden" value="<?php echo $row_producto[2]; ?>" /></td>
                                                    <td><?php echo $newArray[$row_producto[0]]; ?></td>
                                    ...
                                    // $sql_fin = mysql_query("SELECT pd.producto, pd.contactos, pd.visitador from po_p1 p, po_p1_detalle pd where p.id = pd.id and p.ciudad = $valores and p.ciclo = $codigo_ciclo and p.gestion = $codigo_gestion2 and pd.visitador = $val_fun ORDER BY 2 DESC limit 5");
                                    // while($r = mysql_fetch_array($sql_fin)){
                                    //     $newArray[$r[0]] = 1; 
                                    // }
                                    ?>
                                </tbody>
