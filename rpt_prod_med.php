...
{	if($medico=="" && $obj=="" && $filtrado=="")
	{	 $sql="select distinct m.cod_med,m.ap_pat_med,m.ap_mat_med,m.nom_med
		 ...
		 where m.cod_ciudad='$global_agencia' and m.cod_med=c.cod_med and (p.cod_med=m.cod_med or n.cod_med=m.cod_med) and c.codigo_linea=$global_linea order by m.ap_pat_med";
	}
	else
	{	$sql="select distinct m.cod_med,m.ap_pat_med,m.ap_mat_med,m.nom_med
	...
	echo "<center><table border='0' class='textotit'><tr><th>Productos Objetivo y Filtrados x Medico</th></tr></table></center><br>";
	...
	while($dat=mysql_fetch_array($resp))
	{
		...
		$sql_prod_objetivo="select p.codigo_muestra, mm.descripcion, mm.presentacion
						...
						where p.cod_med=m.cod_med and p.cod_med='$cod' and p.codigo_linea='$global_linea' and mm.codigo=p.codigo_muestra and m.codigo_visitador='$global_visitador'
		...
		$sql_muestras_negadas="select mn.codigo_muestra, mm.descripcion, mm.presentacion
						...
						where mn.cod_med=m.cod_med and mn.cod_med='$cod' and mn.codigo_linea='$global_linea' and mm.codigo=mn.codigo_muestra and m.codigo_visitador='$global_visitador'
						order by mm.descripcion, mm.presentacion";
		...
		echo "<tr><td align='center'>$cod</td><td align='center' class='texto'>$nombre_completo</td><td align='center'>&nbsp;$especialidad</td><td align='center'>$cadena_prod_objetivo</td><td align='center'>$cadena_prod_filtrado</td></tr>";
...
{	$sql_cab_obj="select descripcion, presentacion from muestras_medicas where codigo='$obj'";
	...
	echo "<center><table border='0' class='textotit'><tr><th>Medicos que tienen como producto objetivo: $descripcion_prod_obj $presentacion_prod_obj</th></tr></table></center><br>";
...
{	$sql_cab_filtrado="select descripcion, presentacion from muestras_medicas where codigo='$filtrado'";
	...
	echo "<center><table border='0' class='textotit'><tr><th>Medicos que tienen como producto filtrado: $descripcion_prod_filtrado $presentacion_prod_filtrado</th></tr></table></center><br>";
	echo "<center><table border='1' class='textomini' width='80%' cellspacing='0'>";
	echo "<tr><th>Codigo</th><th>Nombre</th><th>Especialidades</th></tr>";		
	$sql_medicos="select distinct m.cod_med,m.ap_pat_med,m.ap_mat_med,m.nom_med
