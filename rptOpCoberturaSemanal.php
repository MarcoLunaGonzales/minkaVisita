...
{	var rpt_visitador,rpt_gestion, rpt_ciclo;
	...
	var rpt_semana=new Array();
	var rpt_nombresemana=new Array();
	...
	j=0;
	for(i=0;i<=f.rpt_semana.options.length-1;i++)
	{	if(f.rpt_semana.options[i].selected)
		{	rpt_semana[j]=f.rpt_semana.options[i].value;
			rpt_nombresemana[j]=f.rpt_semana.options[i].innerHTML;
			j++;
		}
	}
	
	window.open('rptCoberturaResumen.php?rpt_nombreterritorio='+rpt_nombreterritorio+'&rpt_territorio='+rpt_territorio+'&rpt_gestion='+rpt_gestion+'&rpt_ciclo='+rpt_ciclo+'&rpt_categoria='+rpt_categoria+'&rpt_semana='+rpt_semana+'&rpt_nombresemana='+rpt_nombresemana,'','scrollbars=yes,status=no,toolbar=no,directories=no,menubar=no,resizable=yes,width=1000,height=800');
	return(true);
}
...
echo "<center><table class='textotit'><tr><th>Reporte de Cobertura Resumen</th></tr></table><br>";
echo"<form method='post'>";
	...
	echo "<tr><th align='left'>Semana</th>
	<td>
		<select name='rpt_semana' id='rpt_semana' size='5'>
