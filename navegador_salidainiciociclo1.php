...
echo "<script language='Javascript'>
	...
	function validar(f)
	{
		for(j=0;j<=f.length-1;j++)
		{
			...
			{	if((f.elements[j].value) > (f.elements[j+1].value))
				{	alert('No se pueden sacar cantidades mayores al stock.');	
					f.elements[j].focus();
					return(false);	
				}
			}
		}   
		//location.href='guarda_salidaalmacenes.php?vector_material='+vector_material+'&vector_fechavenci='+vector_fechavenci+'&vector_cantidades='+vector_cantidades+'&fecha='+fecha+'&tipo_ingreso='+tipo_ingreso+'&observaciones='+observaciones+'&cantidad_material='+cantidad_material+'&vector_tipomaterial='+vector_tipomaterial+'';
	}
	</script>";
...
echo "<tr><th>&nbsp;</th><th>Material</th><th>Cantidad Planificada</th><th>Cantidad Unitaria</th><th>Stock</th><th>Cantidad requerida Nacional</th></tr>";
$indice_detalle=1;
for($i=0;$i<=$indice_m_m;$i++)
