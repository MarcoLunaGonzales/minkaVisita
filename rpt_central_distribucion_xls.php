...
echo "<table border='0' class='textotit' align='center'><tr><th>Reporte Distribución x Ciclo<br>Gestion: $rpt_gestion Ciclo: $rpt_ciclo $cad_territorio</th></tr></table></center><br>";
...
{	$sql_productos="select distinct(codigo_producto), grupo_salida from distribucion_productos_visitadores 
	...
	echo "<table border=1 class='texto' cellspacing=0 cellpading=0>";
...
{	$sql_productos="select distinct(codigo_producto), grupo_salida from distribucion_productos_visitadores 
	...
	{	$cod_visitador=$dat_visitadores[0];
		...
		echo "<th colspan=2>$nombre_visitador<table border=1 cellspacing=0 cellpading=0 class='texto' width='100%'>
		<tr><td width='50%'>CP</td><td width='50%'>CD</td></tr></table></th>";
	}	
	while($dat_productos=mysql_fetch_array($resp_productos))
	{	$cod_prod=$dat_productos[0];
