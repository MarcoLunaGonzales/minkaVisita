...
echo "<script language='JavaScript'>
		...
		{	var visitador,gestion_rpt, ciclo_rpt;
			...
			window.open('rpt_regional_prod_visitador.php?linea_rpt='+linea_rpt+'&visitador='+visitador+'&gestion_rpt='+gestion_rpt+'&ciclo_rpt='+ciclo_rpt+'','','scrollbars=yes,status=no,toolbar=no,directories=no,menubar=no,resizable=yes,width=1000,height=800');			
			return(true);
		}
		...
		{	var visitador,gestion_rpt, ciclo_rpt;
			...
			gestion_rpt=f.gestion_rpt.value;
			ciclo_rpt=f.ciclo_rpt.value;
			window.open('rpt_regional_prod_visitador_xls.php?linea_rpt='+linea_rpt+'&visitador='+visitador+'&gestion_rpt='+gestion_rpt+'&ciclo_rpt='+ciclo_rpt+'','','scrollbars=yes,status=no,toolbar=no,directories=no,menubar=no,resizable=yes,width=1000,height=800');			
			return(true);
		}
		</script>";
require("conexion.inc");
require('estilos_regional_pri.inc');
echo "<center><table class='textotit'><tr><th>Reporte Muestras planificadas x Visitador</th></tr></table><br>";
echo"<form method='post'>";
	...
	where f.cod_cargo=c.cod_cargo and f.codigo_funcionario=fl.codigo_funcionario and f.cod_cargo='1011' and f.estado=1 and fl.codigo_linea='$linea_rpt' and f.cod_ciudad='$global_agencia' and f.cod_ciudad=ci.cod_ciudad order by ci.descripcion,f.paterno,c.cargo";
