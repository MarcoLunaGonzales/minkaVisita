...
$resp_mod=mysql_query($sql_mod);
	...
	if($dir2==1)
	{
		...
		{
			$sql1="insert into direcciones_medicos values('$cod_med','$da[17]','$da[15]','Consultorio II')";
			$resp1=mysql_query($sql1);
		}
	...
	if($dir3==1)
	{	$sql_aux="select * from direcciones_medicos where cod_med=$cod_med and descripcion='Institucional'";
		$resp_aux=mysql_query($sql_aux);
		$num_filas=mysql_num_rows($resp_aux);
		if($num_filas==1)
		{
			$sql1="update direcciones_medicos set cod_zona='$da[21]', direccion='$da[19]' where cod_med=$cod_med and descripcion='Institucional'";
	  		$resp1=mysql_query($sql1);
		}
		else
		{
			$sql1="insert into direcciones_medicos values('$cod_med','$da[21]','$da[19]','Institucional')";
			$resp1=mysql_query($sql1);
		}
	}
	else
	{
		$sql1="delete from direcciones_medicos where cod_med=$cod_med and descripcion='Institucional'";
	  	$resp1=mysql_query($sql1);
	}
	if($dir_cat==1)
	{
		$sql_aux="select * from especialidades_medicos where cod_med=$cod_med and descripcion='Sub-especialidad'";
		$resp_aux=mysql_query($sql_aux);
		$num_filas=mysql_num_rows($resp_aux);
		if($num_filas==1)
		{
			$sql1="update especialidades_medicos set cod_especialidad='$da[24]' where cod_med=$cod_med and descripcion='Sub-especialidad'";
	  		$resp1=mysql_query($sql1);
		}
		else
		{
			echo $da[24];
			$sql1="insert into especialidades_medicos values('$cod_med','$da[24]','Sub-especialidad')";
			$resp1=mysql_query($sql1);
		}
	}
	else
	{	$sql1="delete from especialidades_medicos where cod_med=$cod_med and descripcion='Sub-especialidad'";
		$resp1=mysql_query($sql1);
	}
