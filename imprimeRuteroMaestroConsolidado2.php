...
function imprimeRuteroMaestro($codigoGestion, $codigoCiclo, $tipoRuteroRpt, $visitador, $idtable) {
    ...
    $sqlRuteros = "SELECT r.cod_rutero, r.codigo_linea, l.nombre_linea from $tabla1 r, lineas l, funcionarios_lineas fl where r.codigo_linea=l.codigo_linea and r.cod_visitador = fl.codigo_funcionario and r.codigo_linea = fl.codigo_linea and r.codigo_ciclo = $codigoCiclo and r.codigo_gestion = $codigoGestion and r.cod_visitador = $visitador";
    ...
    <table border="1" align='center' width='90%' id="<?php echo $idtable ?>" class='texto'>
        ...
        while ($row_espe = mysql_fetch_array($sql_espe)) {
            $cadena_imprimir .= "<tr>";
            $cadena_imprimir .= "<td>$row_espe[1]</td>";
            $respRuteros = mysql_query($sqlRuteros);
            ...
            while($row_l = mysql_fetch_array($respRuteros)){
                ...
                while ($dat_medicos = mysql_fetch_array($resp_medicos)) {
                    ...
                    $sql_cant_contactos = "SELECT rmd.cod_med from $tabla1 rmc, $tabla2 rm, $tabla3 rmd, medicos m where rmc.cod_rutero=rm.cod_rutero and rm.cod_contacto=rmd.cod_contacto and m.cod_med=rmd.cod_med and rmc.cod_rutero='$cod_rutero_f' and rmc.codigo_linea='$linea_casilla'and rmd.cod_visitador='$visitador' and rmd.cod_especialidad='$row_espe[0]' and rmd.cod_med='$cod_med' and rmd.categoria_med = '$dat_medicos[4]'";
