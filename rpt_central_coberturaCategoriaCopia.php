...
$sql_visitador="SELECT f.codigo_funcionario, f.paterno, f.materno, f.nombres from funcionarios f, funcionarios_lineas fl where f.codigo_funcionario=fl.codigo_funcionario and f.estado=1 and fl.codigo_linea='$rpt_linea' and f.cod_ciudad='$rpt_territorio' and f.cod_cargo='1011'and f.codigo_funcionario in ($rpt_visitador) order by f.paterno, f.materno";
$resp_visitador=mysql_query($sql_visitador);
echo "<center><table border='0' class='textotit'><tr><th>Cobertura x Ciclo, Visitador y Categoria<br>
...
while($dat_visitador=mysql_fetch_array($resp_visitador)) {
	...
	and rd.categoria_med='A' and rc.codigo_linea = $rpt_linea";
    ...
    $sql_contactos_maestro="SELECT count(*) from rutero_maestro_cab_aprobado rc, rutero_maestro_aprobado r, rutero_maestro_detalle_aprobado rd where rc.cod_rutero=r.cod_rutero and rd.cod_contacto=r.cod_contacto and rc.cod_visitador='$codigo_visitador' and rc.codigo_ciclo='$rpt_ciclo' and rc.codigo_gestion='$rpt_gestion' and rd.categoria_med='B' and rc.codigo_linea = $rpt_linea";
    $resp_contactos_maestro=mysql_query($sql_contactos_maestro);
    $dat_maestro=mysql_fetch_array($resp_contactos_maestro);
    $numero_contactos_maestroB=$dat_maestro[0];

    $sql_contactos_maestro="SELECT count(*) from rutero_maestro_cab_aprobado rc, rutero_maestro_aprobado r, rutero_maestro_detalle_aprobado rd where rc.cod_rutero=r.cod_rutero and rd.cod_contacto=r.cod_contacto and rc.cod_visitador='$codigo_visitador' and rc.codigo_ciclo='$rpt_ciclo' and rc.codigo_gestion='$rpt_gestion' and rd.categoria_med='C' and rc.codigo_linea = $rpt_linea";
    $resp_contactos_maestro=mysql_query($sql_contactos_maestro);
    $dat_maestro=mysql_fetch_array($resp_contactos_maestro);
    ...
    $sql_contactos_ejecutado="SELECT count(rd.cod_contacto) from rutero_detalle rd, rutero r where r.cod_ciclo='$rpt_ciclo' and r.codigo_gestion='$rpt_gestion' and r.cod_visitador='$codigo_visitador'and r.codigo_linea='$rpt_linea' and r.cod_contacto=rd.cod_contacto and rd.estado='1' and rd.categoria_med='A'";
    ...
    $sql_contactos_ejecutado="SELECT count(rd.cod_contacto) from rutero_detalle rd, rutero r where r.cod_ciclo='$rpt_ciclo' and r.codigo_gestion='$rpt_gestion' and r.cod_visitador='$codigo_visitador'and r.codigo_linea='$rpt_linea' and r.cod_contacto=rd.cod_contacto and rd.estado='1' and rd.categoria_med='B'";
    $resp_contactos_ejecutado=mysql_query($sql_contactos_ejecutado);
    $dat_ejecutado=mysql_fetch_array($resp_contactos_ejecutado);
    ...
    $sql_contactos_ejecutado="SELECT count(rd.cod_contacto) from rutero_detalle rd, rutero r where r.cod_ciclo='$rpt_ciclo' and r.codigo_gestion='$rpt_gestion' and r.cod_visitador='$codigo_visitador'and r.codigo_linea='$rpt_linea' and r.cod_contacto=rd.cod_contacto and rd.estado='1' and rd.categoria_med='C'";
    $resp_contactos_ejecutado=mysql_query($sql_contactos_ejecutado);
    $dat_ejecutado=mysql_fetch_array($resp_contactos_ejecutado);
    ...
    $sql_baja_dias="SELECT distinct(b.dia_contacto), b.turno from baja_dias b, baja_dias_detalle bd, baja_dias_detalle_visitador bdv where b.codigo_baja=bd.codigo_baja and bd.codigo_baja=bdv.codigo_baja and b.ciclo='$rpt_ciclo' and b.gestion='$rpt_gestion' and bd.codigo_linea='$rpt_linea'and  bdv.codigo_visitador='$codigo_visitador'";
    ...
    while($dat_baja_dias=mysql_fetch_array($resp_baja_dias)) {   
        ...
        $sql_nro_contactos_baja="SELECT count(r.cod_contacto), rd.categoria_med from rutero r, rutero_detalle rd where r.cod_contacto=rd.cod_contacto and r.cod_ciclo='$rpt_ciclo' and r.codigo_gestion='$rpt_gestion' and r.cod_visitador='$codigo_visitador'and r.dia_contacto='$dia_contacto_baja' and turno='$turno_contacto_baja' and r.codigo_linea='$rpt_linea' group by rd.categoria_med";
