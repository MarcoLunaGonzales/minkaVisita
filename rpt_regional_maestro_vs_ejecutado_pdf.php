...
{	$sql_contactos_maestro="select count(rd.cod_contacto) from rutero_detalle_utilizado rd, rutero_utilizado r 
	...
	echo "<center><table border='0' class='textotit'><tr><th>Rutero Maestro vs. Rutero Ejecutado<br>Formato Resumido<br>Visitador $nombre_visitador</th></tr></table></center><br>";
...
{	$sql_medicos_asignados="select ma.cod_med, m.ap_pat_med, m.ap_mat_med, m.nom_med from medico_asignado_visitador ma, medicos m
	...
	echo "<center><table border='0' class='textotit'><tr><th>Rutero Maestro vs. Rutero Ejecutado<br>Formato Detallado<br>Visitador $nombre_visitador</th></tr></table></center><br>";
	...
	$indice_tabla=1;
	echo "<table border=1 width='80%' class='texto' align='center' cellspacing=0><tr><th>&nbsp;</th><th>Medico</th><th>Contactos Rutero Maestro</th><th>Contactos Rutero Ejecutado</th><th>Diferencia</th></tr>";
	...
	{	$codigo_medico=$dat_medicos_asignados[0];
		...
		where r.cod_ciclo='$ciclo_rpt' and r.codigo_gestion='$gestion_rpt' and r.cod_contacto=rd.cod_contacto and rd.cod_med='$codigo_medico'";
		$resp_contactos_maestro=mysql_query($sql_contactos_maestro);
		...
		{	if($numero_contactos_maestro!=$numero_contactos_ejecutado)
			...
			echo "<tr bgcolor=$fondo_fila><td align='center'>$indice_tabla</td><td>$nombre_medico</td><td align='center'>$numero_contactos_maestro</td><td align='center'>$numero_contactos_ejecutado</td><td align='center'>$diferencia</td></tr>";
			$indice_tabla++;
		}
	}
	echo "</table>";
