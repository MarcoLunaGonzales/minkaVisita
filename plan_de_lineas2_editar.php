...
<head>
    ...
    $(document).ready(function() {
        ...
        $('#enviar').click(function(){
            ...
            $.getJSON("ajax/plan_lineas/editar.php",{
                "cadena" : cadena,
                "id_cab" : <?php echo $id ?>
            },response);
                ...
                foreach ($cadena_explode as $datos) {
                    ...
                    <div class="three columns end">
                        <p class="title">
                            ...
                            <?php $ciudad_codigo = $datos_explode[0]; ?>
                        </p>
                        <table border="1">
                            <?php
                                        ...
                                        <ul class="lista_espe">
                                            ...
                                            if ($tiene_lineas == 0) {
                                                ...
                                                while ($row_li = mysql_fetch_array($sql_lineas)) {
                                                    ...
                                                    <li> 
                                                        <?php 
                                                        $sql_ver_checked = mysql_query("SELECT COUNT(*) from plan_lineas_detalle pd, plan_lineas p 
                                                            where p.id = pd.id and p.id_cab = $id and p.ciudad = $ciudad_codigo and pd.especialidad = '$especialidades' and pd.linea = $row_li[0]");
                                                        if (mysql_result($sql_ver_checked, 0, 0) > 0) {
                                                            ?>
                                                            <input type="checkbox" value="<?php echo $especialidades.",".$row_li[0]; ?>" checked /> <?php echo $row_li[1]; ?>        
                                                            <?php
                                                        }else{
                                                            ?>
                                                            <input type="checkbox" value="<?php echo $especialidades.",".$row_li[0]; ?>"  /> <?php echo $row_li[1]; ?>
                                                            <?php
                                                        }
                                                        ?>
