...
$sql_visitadores="select f.codigo_funcionario, f.paterno, f.materno, f.nombres, rc.codigo_linea, rc.codigo_linea, f.cod_zeus, f.cod_ciudad
	...
	where f.codigo_funcionario=rc.cod_visitador and f.cod_ciudad in (118) and f.cod_zeus<>0 and 
	rc.codigo_ciclo='$rpt_ciclo' and rc.codigo_gestion='$global_gestion' and f.codigo_funcionario in (1425) order by f.cod_ciudad";
...
//$indice_reporte    = 1;
$sqlXXX="select IFNULL(max(id_boleta+1),1) from boletas_visita_cabXXX_copy";
    ...
    while ($dat = mysql_fetch_array($resp)) {
        ...
        while ($dat1 = mysql_fetch_array($resp1)) {
			...
			$numFilasPersonalizada=1;
                ...
                $sql_insert_reporte_cab = "INSERT into boletas_visita_cabXXX_copy (id_boleta, id_visitador_hermes, id_visitador_zeus, territorio, visitador, medico, linea, 
				...
				while ($dat_parrilla_detalle = mysql_fetch_array($resp_parrilla_detalle)) {
					...
					$insert_reporte_det = "INSERT into boletas_visita_detalleXXX_copy (id_boleta, orden, muestra, cantidad_muestra, material, 
				...
				while($datPGE=mysql_fetch_array($respPGE)){
					...
					$insert_reporte_det = "INSERT into boletas_visita_detalleXXX_copy values($indice_reporte,$orden_promocional,'$muestra','$cant_muestra','$material','$cant_material',2,'$codMuestra')";
...
//verificar que maximo hayan 15 items.
	$sqlDelMax="update boletas_visita_detalleXXX_copy set material='--', cantidad_material='0' where material='Sin Material Apoyo'";
	$respDelMax=mysql_query($sqlDelMax);

	$sqlDelMax="delete from boletas_visita_detalleXXX_copy where orden_promocional>15";
	$respDelMax=mysql_query($sqlDelMax);
