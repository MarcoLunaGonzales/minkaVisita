...
if($cantidadDist==0){
	...
	";
	$sqlUpdParrilla="select pd.codigo_parrilla, pd.prioridad from parrilla p, parrilla_detalle pd where p.codigo_parrilla=pd.codigo_parrilla and 
		p.cod_ciclo=$codCicloDist and p.codigo_gestion=$codGestionDist and p.codigo_linea=$codLineaDist and pd.codigo_muestra='$codProdOrigen'";
	...
	while($datUpdParrilla=mysql_fetch_array($respUpdParrilla)){
		...
		*/
		...
		$sqlActParrilla="update parrilla_detalle set codigo_muestra='$codProdDestino' where codigo_muestra='$codProdOrigen' and
