...
$sql_visitadores="select f.codigo_funcionario, f.paterno, f.materno, f.nombres, rc.codigo_linea, rc.codigo_linea, f.cod_zeus, f.cod_ciudad
	...
	where f.codigo_funcionario=rc.cod_visitador and f.cod_ciudad in ($agencia) and f.cod_zeus<>0 and 
	rc.codigo_ciclo='$rpt_ciclo' and rc.codigo_gestion='$global_gestion' and f.codigo_funcionario in ('$rpt_visitador') and 
	rc.codigo_linea in ('$rptLineaXX')";
...
//$indice_reporte    = 1;
$sqlXXX="select IFNULL(max(id_boleta+1),1) from boletas_visita_cab_copy";
    ...
    while ($dat = mysql_fetch_array($resp)) {
        ...
        while ($dat1 = mysql_fetch_array($resp1)) {
			...
			$numFilasPersonalizada=1;
                ...
                $sql_insert_reporte_cab = "INSERT into boletas_visita_cab_copy (id_boleta, id_visitador_hermes, id_visitador_zeus, territorio, visitador, medico, linea, 
				gestion, id_ciclo, id_gestion, fecha, id_medico, nro_boleta, direccion1, telefono, celular, especialidad, supervisor, estado) 
				...
				'$indice_boleta de $cantidad_total_boletas','$dir_rep1','$telefono_medico', '$celular_medico','$espe','$nombre_supervisor','0')";
				...
				while ($dat_parrilla_detalle = mysql_fetch_array($resp_parrilla_detalle)) {
					...
					$insert_reporte_det = "INSERT into boletas_visita_detalle_copy (id_boleta, orden, muestra, cantidad_muestra, material, 
					cantidad_material, tipo) values ($indice_reporte,$orden_promocional,'$muestra','$cant_muestra','$material','$cant_material',1)";
				...
				while($datPGE=mysql_fetch_array($respPGE)){
					...
					$insert_reporte_det = "INSERT into boletas_visita_detalle_copy values($indice_reporte,$orden_promocional,'$muestra','$cant_muestra','$material','$cant_material',2)";
				...
				if($numero_visita==2){
					...
					while($datBancos=mysql_fetch_array($respBancos)){
						...
						$cant_material=0;
						$insert_reporte_det = "INSERT into boletas_visita_detalle_copy values($indice_reporte,$orden_promocional,'$muestra','$cant_muestra','$material','',3)";
...
//verificar que maximo hayan 15 items.
	$sqlDelMax="update boletas_visita_detalle_copy set material='--', cantidad_material='0' where material='Sin Material Apoyo'";
	$respDelMax=mysql_query($sqlDelMax);

	$sqlDelMax="delete from boletas_visita_detalle_copy where orden_promocional>15";
	$respDelMax=mysql_query($sqlDelMax);
