...
*/
	echo "<script language='JavaScript'>
		...
		variables=new Array(f.length-1);
			...
			location.href='guardar_modi_contactos.php?variables='+variables+'&ciclo_trabajo=$ciclo_trabajo';
			return(true);
		}
		</script>";
	...
	$sql_pre="select dia_contacto,fecha,turno from contactos where cod_contacto=$j_contacto";
	$resp_pre=mysql_query($sql_pre);
	$dat_pre=mysql_fetch_array($resp_pre);
	$diasciclo=$dat_pre[0];
	$fecha=$dat_pre[1];
	$input_FechaConsulta="$fecha[8]$fecha[9]/$fecha[5]$fecha[6]/$fecha[0]$fecha[1]$fecha[2]$fecha[3]";
	$turno=$dat_pre[2];

	$sql_pre2="select orden_visita,cod_med,cod_especialidad,categoria_med,cod_zona
				from contactos_detalle where cod_contacto=$j_contacto order by orden_visita asc";
	...
	//hasta aca
	//formamos el vector de dias y fechas
	...
	echo "<center><table class='textotit'><tr><th>Edición de Contactos<br>Ciclo de Trabajo:$ciclo_trabajo</th></tr></table></center>";
	...
	echo "</select></td>";
	echo "<td>";
	echo "<table border=0><tr><!-- INI fecha con javascript -->
...
for($i=1;$i<=$num_medicos;$i++)
{
	...
	$v_h_especialidad_med=$$h_especialidad_med;
	$v_h_categoria_med=$$h_categoria_med;
	$sql="select cod_med, ap_pat_med, ap_mat_med, nom_med from medicos order by ap_pat_med";
	$res=mysql_query($sql);
