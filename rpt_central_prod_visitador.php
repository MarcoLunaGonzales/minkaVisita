...
require("estilos_reportes_central.inc");
	...
	$sql_muestras_material="select DISTINCT(pd.codigo_muestra) from parrilla p, parrilla_detalle pd, muestras_medicas m
      where p.cod_ciclo='$ciclo_rpt' and p.codigo_gestion='$gestion_rpt' and p.codigo_linea='$global_linea' and p.codigo_parrilla=pd.codigo_parrilla and pd.codigo_muestra=m.codigo order by m.descripcion";
	...
	$sql_muestras_material="select DISTINCT(pd.codigo_material) from parrilla p, parrilla_detalle pd, material_apoyo m
      where p.cod_ciclo='$ciclo_rpt' and p.codigo_gestion='$gestion_rpt' and p.codigo_linea='$global_linea' and p.codigo_parrilla=pd.codigo_parrilla and pd.codigo_material=m.codigo_material order by m.descripcion_material";
	$resp_muestras_material=mysql_query($sql_muestras_material);
	while($dat_m_m=mysql_fetch_array($resp_muestras_material))
	{	$matriz_m_m[$indice_m_m][1]=$dat_m_m[0];
		$indice_m_m++;
	...
	{	$cod_especialidad=$dat_espe[0];
		...
		{	$sql_parrilla="select pd.codigo_muestra, pd.cantidad_muestra, pd.codigo_material, pd.cantidad_material, p.categoria_med
			from parrilla p, parrilla_detalle pd where p.codigo_parrilla=pd.codigo_parrilla and p.codigo_linea='$global_linea' and p.cod_ciclo='$ciclo_rpt'
			and p.codigo_gestion='$gestion_rpt' and p.cod_especialidad='$cod_especialidad'";
