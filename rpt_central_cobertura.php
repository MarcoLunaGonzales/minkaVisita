...
function totales(){
   ...
   var numCols=main.rows[1].cells.length;
	 ...
	 var cobertura=parseFloat((main.rows[numFilas-1].cells[3].innerHTML / main.rows[numFilas-1].cells[2].innerHTML)*100);
	 cobertura=Math.round(cobertura);		
	 main.rows[numFilas-1].cells[4].innerHTML=cobertura+' %';

	var coberturaAct=parseFloat((main.rows[numFilas-1].cells[3].innerHTML / main.rows[numFilas-1].cells[7].innerHTML)*100);
	 coberturaAct=Math.round(coberturaAct);		
	 main.rows[numFilas-1].cells[8].innerHTML=coberturaAct+' %';
}
</script>
...
//esta parte saca el dia de contacto actual
	$sql_dias_ini_fin="select fecha_ini, fecha_fin from ciclos where cod_ciclo='$rpt_ciclo' and codigo_gestion='$rpt_gestion' 
	and codigo_linea='$rpt_linea'";
...
$sql_visitador="select f.codigo_funcionario, f.paterno, f.materno, f.nombres
	...
	f.estado=1 and fl.codigo_linea='$rpt_linea' and f.cod_ciudad='$rpt_territorio' and f.cod_cargo='1011' 
	and f.codigo_funcionario in ($rpt_visitador) order by f.paterno, f.materno";
	$resp_visitador=mysql_query($sql_visitador);
	echo "<center><table border='0' class='textotit'><tr><th>Cobertura x Ciclo y Visitador<br>Territorio: $nombre_territorio<br>Ciclo: $rpt_ciclo</th></tr></table></center><br>";
	$indice_tabla=1;
	...
	{	$codigo_visitador=$dat_visitador[0];
		...
		where r.cod_ciclo='$rpt_ciclo' and r.codigo_gestion='$rpt_gestion' and r.cod_visitador='$codigo_visitador' and r.codigo_linea='$rpt_linea' and r.cod_contacto=rd.cod_contacto";
		...
		$sql_contactos_ejecutado="select count(rd.cod_contacto) from rutero_detalle rd, rutero r 
			where r.cod_ciclo='$rpt_ciclo' and r.codigo_gestion='$rpt_gestion' and r.cod_visitador='$codigo_visitador' and r.codigo_linea='$rpt_linea' and r.cod_contacto=rd.cod_contacto and rd.estado='1'";
		...
		{	$codigo_medico=$dat_medicos[0]; $inicio_baja=$dat_medicos[1]; $fin_baja=$dat_medicos[2];
			...
			where r.cod_contacto=rd.cod_contacto and r.cod_ciclo='$rpt_ciclo' and r.codigo_gestion='$rpt_gestion' and r.cod_visitador='$codigo_visitador'
			and r.codigo_linea='$rpt_linea' and rd.cod_med='$codigo_medico'";
		...
		where b.codigo_baja=bd.codigo_baja and bd.codigo_baja=bdv.codigo_baja and b.ciclo='$rpt_ciclo' and b.gestion='$rpt_gestion' and bd.codigo_linea='$rpt_linea'
		...
		{	$dia_contacto_baja=$dat_baja_dias[0];
			...
			where r.cod_contacto=rd.cod_contacto and r.cod_ciclo='$rpt_ciclo' and r.codigo_gestion='$rpt_gestion' and r.cod_visitador='$codigo_visitador'
			and r.dia_contacto='$dia_contacto_baja' and turno='$turno_contacto_baja' and r.codigo_linea='$rpt_linea'";
