...
$txt_reporte="Fecha de Reporte <strong>$fecha_reporte</strong>";
	...
	echo "<table align='center' class='textotit'><tr><td align='center'>Reporte Kardex de Existencia Fisica<br>Territorio: <strong>$nombre_territorio</strong> Almacen: <strong>$nombre_almacen</strong> Fecha inicio: <strong>$fecha_ini</strong> Fecha final: <strong>$fecha_fin</strong> Tipo de Item: <strong>$nombre_tipoitem</strong> Item: <strong>$nombre_item</strong><br>$txt_reporte</th></tr></table>";
	...
	i.grupo_ingreso=$tipo_item and i.ingreso_anulado=0 and id.cod_material='$rpt_item' and i.fecha<'$fecha_iniconsulta'";
	...
	s.grupo_salida=$tipo_item and s.salida_anulada=0 and sd.cod_material='$rpt_item' and s.fecha<'$fecha_iniconsulta'";
	...
	where i.cod_ingreso_almacen=id.cod_ingreso_almacen and i.cod_almacen='$rpt_almacen' and
	i.grupo_ingreso=$tipo_item and i.ingreso_anulado=0 and id.cod_material='$rpt_item' and i.fecha>='$fecha_iniconsulta' and i.fecha<='$fecha_finconsulta' order by i.fecha";
	...
	where s.cod_salida_almacenes=sd.cod_salida_almacen and s.cod_almacen='$rpt_almacen' and
	s.grupo_salida=$tipo_item and s.salida_anulada=0 and sd.cod_material='$rpt_item' and s.fecha>='$fecha_iniconsulta' and s.fecha<='$fecha_finconsulta' order by s.fecha";
	...
	{	$fecha_consulta=$vector_final_fechas[$indice];
		...
		i.grupo_ingreso=$tipo_item and i.ingreso_anulado=0 and id.cod_material='$rpt_item' and i.fecha='$fecha_consulta'";
		...
		{	$nro_ingreso=$dat_ingresos[0];
			...
			echo "<tr><td align='center'>$fecha_consulta</td><td>Ingreso</td><td align='center'>$nro_ingreso</td><td align='right'>$cantidad_ingreso</td><td align='right'>0</td><td align='right'>$cantidad_kardex</td><td align='left'>$nombre_ingreso</td><td>&nbsp;</td><td>&nbsp;$obs_ingreso</td></tr>";
		}
		//hacemos la consulta para salidas
		$sql_salidas="select s.nro_correlativo, sd.cantidad_unitaria, ts.nombre_tiposalida, s.observaciones, s.territorio_destino, s.cod_salida_almacenes
		...
		s.grupo_salida=$tipo_item and s.salida_anulada=0 and sd.cod_material='$rpt_item' and s.fecha='$fecha_consulta'";
		...
		{	$nro_salida=$dat_salidas[0];
			...
			$sql_visitador="select concat(f.paterno,' ',f.nombres) from funcionarios f, salida_detalle_visitador s
							where s.codigo_funcionario=f.codigo_funcionario and s.cod_salida_almacen=$cod_salida";
			$resp_visitador=mysql_query($sql_visitador);
			$dat_visitador=mysql_fetch_array($resp_visitador);
			$nombre_visitador=$dat_visitador[0];
			$obs_salida="$nombre_visitador - ".$obs_salida;
			...
			echo "<tr><td align='center'>$fecha_consulta</td><td>Salida</td><td align='center'>$nro_salida</td><td align='right'>0</td><td align='right'>$cantidad_salida</td><td align='right'>$cantidad_kardex</td><td align='left'>$nombre_salida</td><td align='left'>$nombre_territorio_destino</td><td>&nbsp;$obs_salida</td></tr>";
		}
	}
