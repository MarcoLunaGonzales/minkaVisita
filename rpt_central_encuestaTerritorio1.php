...
echo "<table border='0' class='textotit' align='center'><tr><th>Cobertura de Encuestas x Territorio<br>
	...
	$sql_espe="select distinct (m.`cod_espe`) from `medicos_a_encuestar2` m where m.`cod_visitador` in (select codigo_funcionario from funcionarios where
			cod_ciudad in  ($rpt_territorio)) and m.cod_espe in ($rpt_espe1) order by m.`cod_espe`";	
	...
	{	$codEspe=$dat_espe[0];
		...
		{	
			...
			$contRegC=0;
			$totalContReg=0;
			$sqlReg="select * from `medicos_a_encuestar2` m, `encuestamedicos2` e 
