...
if($visitador=="")
{	echo "<center><table border='0' class='textotit'><tr><th>Territorio: $nombre_territorio<br>Productos x Visitadores Medicos Gestión: $nombre_cab_gestion Ciclo: $ciclo_rpt</th></tr></table></center><br>";
...
{	$sql_visitador="select paterno, materno, nombres
	...
	{	$cod_especialidad=$dat_espe[0];
		$desc_especialidad=$dat_espe[1];
		$sql_medicos="select DISTINCT (rmd.cod_med), m.ap_pat_med, m.ap_mat_med, m.nom_med, rmd.categoria_med
		from rutero_utilizado rm, rutero_detalle_utilizado rmd, medicos m
		where rm.cod_contacto=rmd.cod_contacto and m.cod_med=rmd.cod_med and rm.codigo_linea='$linea_rpt' and rm.cod_ciclo='$ciclo_rpt' and rm.cod_visitador='$global_visitador' and rmd.cod_especialidad='$cod_especialidad' order by m.ap_pat_med, m.ap_mat_med, m.nom_med";
		$resp_medicos=mysql_query($sql_medicos);
		$num_filas=mysql_num_rows($resp_medicos);
		$numero_total_medicos=$numero_total_medicos+$num_filas;
		$numero_a=0;
		...
		{	$cod_med=$dat_medicos[0];
			$sql_cant_contactos="select rmd.cod_med
			from rutero_maestro rm, rutero_maestro_detalle rmd, medicos m
			where rm.cod_contacto=rmd.cod_contacto and m.cod_med=rmd.cod_med and rm.codigo_linea='$linea_rpt' and rm.cod_visitador='$global_visitador' and rm.cod_ciclo='$ciclo_rpt' and rmd.cod_especialidad='$cod_especialidad' and rmd.cod_med='$cod_med'";
			$resp_cant_contactos=mysql_query($sql_cant_contactos);
			$num_contactos=mysql_num_rows($resp_cant_contactos);
			$categoria_med=$dat_medicos[4];
			if($categoria_med=="A")
