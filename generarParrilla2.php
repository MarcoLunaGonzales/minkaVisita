...
$codigoLineaMkt="1023";
//$codigoLineaMkt="1009,1022,1023,1037,1038,1032";
//$codigoLineaMkt="1039,1040,1041,1042,1043,1044,1045,1046";

//$codigoLineaMkt="1044,1045";

//$sql_ciudades = mysql_query("SELECT cod_ciudad from ciudades where cod_ciudad = 102");
...
while ($row_cod_ciudad = mysql_fetch_array($sql_ciudades)) {
	...
	$sqlLinea="select l.codigo_linea from lineas l where l.estado=1 and l.linea_promocion=1 and codigo_linea in ($codigoLineaMkt)";
	...
	while($datLinea=mysql_fetch_array($respLinea)){
		...
		$sqlGrilla="select gd.cod_especialidad, gd.cod_linea_visita, gd.cod_categoria, gd.frecuencia from grilla g, grilla_detalle gd 
			...
			g.estado=1 and gd.frecuencia>0 and gd.cod_linea_visita=0 and cod_categoria in ('A','B','C')";
		
		echo "sqlGrilla ".$sqlGrilla;
		...
		while($datGrilla=mysql_fetch_array($respGrilla)){
			...
			$sqlAsig="select ifnull(max(a.contacto),0) from asignacion_productos_excel_detalle a where a.especialidad='$codEspe' 
				and a.linea_mkt='$codigoLinea' and a.ciudad='$codCiudad' and a.linea=0";
			//echo "sqlAsig ".$sqlAsig;
			...
			if($numeroMed>0){
				for($ii=1; $ii<=$frecuencia; $ii++){
					...
					$sqlDet="select p.posicion, p.producto from asignacion_productos_excel_detalle p where p.ciudad=$codCiudad and 
						p.especialidad='$codEspe' and p.linea_mkt=$codigoLinea and contacto=$ii order by posicion";
					...
					if($numeroCont>0){
						...
						echo "no hay parrilla para $ii $codCiudad $codigoLinea $codEspe $codCat";
					}
				}
			}
		}
...
echo "</table>";
echo "PARRILLA CARGADA LINEA $codigoLineaMkt";
?>