...
Territorio: $rpt_nombreterritorio   Cat. Medico: $rpt_categoria<br>Gestion: $nombreGestion Ciclo: $rpt_ciclo
...
for($jj=1; $jj<=5; $jj++){
	...
	for ($ii=1; $ii<=5; $ii++){
		...
		while($datAreas=mysql_fetch_array($respAreas)){
			...
			rc.cod_visitador in (select codigo_funcionario from funcionarios where cod_ciudad in ($codArea)) and rc.codigo_ciclo='$rpt_ciclo' and 
			...
			where rc.cod_visitador in (select codigo_funcionario from funcionarios where cod_ciudad in ($codArea)) 
	...
	echo "<tr bgcolor='#FE9A2E'><th>&nbsp;</th><th>&nbsp;</th><TH>Semana $jj</TH><th>$totalContactosMaestroSemana</th><th>$totalContactosEjecutadoSemana</th>
...
echo "<tr><th>&nbsp;</th><TH>TOTALES</TH><TH>TOTALES</TH><th>$totalContactosMaestro</th><th>$totalContactosEjecutado</th>
