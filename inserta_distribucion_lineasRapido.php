...
{	$sql_visitador="select fl.codigo_funcionario,f.cod_ciudad,f.codigo_lineaclave
	...
	{	
		...
		$sqlRutero="select count(DISTINCT(rmd.cod_med)), rmd.cod_especialidad, rmd.categoria_med
		...
		and rmc.cod_visitador='$codVisitador' group by rmd.cod_especialidad, rmd.categoria_med";
		
		echo "SQL RUTERO:       ".$sqlRutero."<BR><BR><BR>";
		...
		while($datRutero=mysql_fetch_array($respRutero)){	
			...
			echo "$codEspe $codCat $cantContactos $codVisitador<br>";
			...
			echo "<br> SQL PARRILLA  ".$sqlParrilla."<BR><BR><BR>";
