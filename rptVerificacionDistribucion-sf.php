...
echo "<center><h2>Reporte de Verificacion Distribucion<br>Ciclo: $codCiclo Gestion: $codGestion</h2></center><br>";
...
while ($datLineas = mysql_fetch_array($respLineas)) {
    ...
    while ($datVis = mysql_fetch_array($respVis)) {
        ...
        echo "<tr><td>Cod Med</td><td>Medico</td><td>Esp.</td><td>Categoria</td><td>Contactos Rutero</td><td>Nro. Parrillas</td><td>Observaci&oacute;n</td><td>Linea</td></tr>";
        ...
        if ($numFilasRutero == 0) {
            ...
            echo "<tr><td colspan='6'><span style='color:#ff0000;font-size=40px'>EL FUNCIONARIO NO TIENE RUTERO APROBADO PARA EL CICLO Y GESTION!!!!!!!!!!</span></td><td>$obs</td></tr>";
        } else {
            while ($datRutero = mysql_fetch_array($respRutero)) {
				...
				and lv.`codigo_funcionario`=$codVisitador and le.`cod_especialidad`='$codEspecialidadMed' and l.`codigo_linea`=$codLinea and lv.codigo_ciclo = $codCiclo and lv.codigo_gestion = $codGestion";
...
//                echo $sqlParrilla."<br />";
                ...
                echo "<tr><td>$codMed</td><td>$nombreMed</td><td>$codEspecialidadMed</td><td>$categoriaMed</td>
					<td>$nroContactos</td><td>$nroParrillas</td><td>$obs</td><td>$nombreLineaVisita</td></tr>";
            }
        }
    }
}
