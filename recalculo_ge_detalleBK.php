    ...
    $(document).ready(function() {
        ...
        $("#boton").click(function(){
            ...
            // alert(valores);
             /*$.ajax({
                    type: "POST",
                    url: "ajax/grupos_especiales/enviar.php",
                    dataType : 'json',
                    data: { 
                        valores : valores,
            ...
            $.getJSON("ajax/grupos_especiales/enviar.php",{
                "valores" : valores,
                "ciclo": <?php echo $codigo_ciclo; ?>,
                "gestion": <?php echo $codigo_gestion; ?>,
                "territorios": '<?php echo $territorios; ?>'
        ...
        <section role="main">
            <div class="row">
                ...
                <div class="twelve columns">
                    ...
                    //$sql_ge = mysql_query("SELECT g.codigo_grupo_especial, g.nombre_grupo_especial from grupo_especial g where g.codigo_linea in ($global_linea) and g.agencia='$cod_ciudad_final' order by g.nombre_grupo_especial ");
                    ...
                    //while ($row_ge = mysql_fetch_array($sql_ge)) {
						//$codGEX=$row_ge[0];
                        $txtMedico="SELECT distinct(concat(m.ap_pat_med, ' ',m.ap_mat_med,' ',m.nom_med)) as nom, c.cod_especialidad,c.categoria_med,m.cod_med 
						from grupo_especial g, grupo_especial_detalle gd, medicos m, categorias_lineas c where g.codigo_grupo_especial = gd.codigo_grupo_especial 
						and gd.cod_med = m.cod_med and c.codigo_linea = g.codigo_linea and c.cod_med = gd.cod_med and g.agencia = '$cod_ciudad_final' 
						ORDER BY nom, c.cod_especialidad";
						$sql_medico = mysql_query($txtMedico);
                        
                        //echo("SELECT concat(m.ap_pat_med, ' ',m.ap_mat_med,' ',m.nom_med) as nom, c.cod_especialidad,c.categoria_med,m.cod_med from grupo_especial g, grupo_especial_detalle gd, medicos m, categorias_lineas c where g.codigo_grupo_especial = gd.codigo_grupo_especial and gd.cod_med = m.cod_med and c.codigo_linea = g.codigo_linea and c.cod_med = gd.cod_med and g.codigo_linea = '$global_linea'and g.agencia = '$cod_ciudad_final' AND g.codigo_grupo_especial = $row_ge[0] ORDER BY g.nombre_grupo_especial, nom, c.cod_especialidad")."<br />";
                        ...
                        <div class="six columns end valores">
                            ...
                            <table border="1" id="<?php echo $row_ge[0] . "-" . $cod_ciudad_final ?>" width='100%'>
                                <!--input type="hidden" value="<?php //echo $cod_ciudad_final."|"; ?>" /-->
                                <thead>
                                    <tr>
                                        <th>Ruc</th>
                                        <th>M&eacute;dico</th>
                                ...
                                <tbody>
                                    ...
                                    while ($row_medicos = mysql_fetch_array($sql_medico)) {
										$codigoMedicoGE=$row_medicos[3];
										$codigo_visitador_consulta = 1;
                                        ?>
                                        <?php
										$sqlGEMed="select g.nombre_grupo_especial, g.codigo_grupo_especial from grupo_especial g, grupo_especial_detalle gd 
										where g.codigo_grupo_especial=gd.codigo_grupo_especial and gd.cod_med=$codigoMedicoGE order by 1";
										$respGEMed=mysql_query($sqlGEMed);
										$cadenaGruposE="";
										$codigosGruposE="";
										while($datGEMed=mysql_fetch_array($respGEMed)){
											$cadenaGruposE=$cadenaGruposE." ".$datGEMed[0];
											$codigosGruposE=$codigosGruposE."-".$datGEMed[1];
										}
										$codigosGruposE=substr($codigosGruposE,1);
                                        ...
                                        $sqlVis = "SELECT DISTINCT rd.cod_visitador , concat(f.paterno,' ',f.materno,' ',f.nombres) as nom 
											...
											f.codigo_funcionario=rd.cod_visitador AND rd.cod_med=$row_medicos[3] AND 
											rc.codigo_ciclo=$codigo_ciclo AND rc.codigo_gestion=$global_gestion and 
											rc.codigo_linea in  ($global_linea) and f.estado = 1 ORDER BY nom ASC ;";
                                        ...
                                        if ($numero_visitadores == 0) {
                                            /*$sqlNoRut="select f.codigo_funcionario, concat(f.paterno, ' ', f.materno, ' ', f.nombres, '***') 
											...
											f.cod_ciudad='$cod_ciudad_final' and fl.codigo_linea='$global_linea'";*/
											$sqlNoRut="select f.codigo_funcionario, concat(f.paterno, ' ', f.nombres, '***') 
											from funcionarios f
											where f.cod_cargo=1011 and f.estado=1 and
											f.cod_ciudad='$cod_ciudad_final'";
											//echo $sqlNoRut;
											$respNoRut=mysql_query($sqlNoRut);
											$cadena_visitador = mysql_result($respNoRut, 0, 1);
                                            $codigo_visitador = mysql_result($respNoRut, 0, 0);
											$cadena_visitador = "<span style='color:blue'>$cadena_visitador</span>";
											
											
											$cadena_visitador_reemplazo = '';
											$respNoRut=mysql_query($sqlNoRut);
                                            while ($datNoRut = mysql_fetch_array($respNoRut)) {
                                                /*if ($codigo_visitador == $datNoRut[0]) {
                                                    $cadena_visitador_reemplazo .= "";
                                                } else {
                                                    $cadena_visitador_reemplazo .= "<div class='cajita' name='$datNoRut[1]' cod='$datNoRut[0]'><span style='color:blue'>" . $datNoRut[1] . "</span></div>";
                                                }*/
												$cadena_visitador_reemplazo .= "<div class='cajita' name='$datNoRut[1]' cod='$datNoRut[0]|'><span style='color:blue'>" . $datNoRut[1] . "</span></div>";
                                            }
										}
                                        if ($numero_visitadores == 1) {
											...
											//$cadena_visitador_reemplazo = "<span style='color:green'>No tiene reemplazos</span>";
										...
										$sqlVeriExiste="select gd.cod_visitador,  
										 ...
										 and g.ciclo=$codigo_ciclo and gd.cod_med=$codigoMedicoGE";
										...
										if($banderaExiste==1){
												...
												$cadena_visitador = "<span style='color:red'>$cadena_visitador</span>";
										}
                                        ...
                                        <tr>
                                            <td>
												...
												<input type="hidden" value="<?php echo $codigosGruposE; ?>" />
                                                <?php if($codigo_visitador_consulta > 0) { ?>
                                                    <input type="hidden" value="<?php echo $row_medicos[3] ?>" />
                                                <?php } ?>
                                            </td>
                                            <td><?php echo $row_medicos[0].'('.$row_medicos[1].')'; ?></td>
                                            <td><?php echo $cadenaGruposE; ?></td>
                                            <td>
                                                <div class="cambio">
