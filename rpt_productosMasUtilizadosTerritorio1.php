...
for($i=1;$i<=4;$i++){
	...
	while($datTerritorio=mysql_fetch_array($respTerritorio)){
		...
		echo "<th colspan='2'>$nombreCiudad</th>";
		
	}
	echo "<th colspan='2'>Total Pais</th></tr>";
	...
	while($dat=mysql_fetch_array($resp)){
		...
		$totalMedicosNacional=0;
		while($datTerritorio=mysql_fetch_array($respTerritorio)){
			$codCiudad=$datTerritorio[0];
			$nombreCiudad=$datTerritorio[1];
			
			$sqlProdTerritorio="select count(e.`frecuencia`) from `encuestamedicos2` e, `medicos_a_encuestar2` me, medicos m 
			...
			$sqlTotalMed="select count(*) from `medicos_a_encuestar2` me, `medicos` m 
				where m.`cod_med`=me.`cod_med` and me.`cod_espe` in ($rpt_espe1) and 
					me.`cod_cat` in ($rpt_cat1) and m.`cod_ciudad` in ($codCiudad)";
			$respTotalMed=mysql_query($sqlTotalMed);
			$totalMedicos=mysql_result($respTotalMed,0,0);
			$totalMedicosNacional=$totalMedicosNacional+$totalMedicos;
			
			$porcentajeFrec=($frecuenciaProdTerritorio/$totalMedicos)*100;
			$porcentajeFrec=redondear2($porcentajeFrec);
			
			echo "<th>$frecuenciaProdTerritorio</th><th>$porcentajeFrec %</th>";
		}
		$porcentajeTotal=redondear2(($frecuencia/$totalMedicosNacional)*100);
		echo "<th>$frecuencia</th><th>$porcentajeTotal %</th></tr>";
		$indice++;
	}	
