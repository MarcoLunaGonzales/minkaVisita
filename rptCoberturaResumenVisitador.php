...
for ($ii=1; $ii<=5; $ii++){
	...
	$sqlAreas="select c.cod_ciudad, c.descripcion from ciudades c where c.cod_ciudad in ($rpt_territorio) order by c.descripcion";
	$respAreas=mysql_query($sqlAreas);
	while($datAreas=mysql_fetch_array($respAreas)){
		$codArea=$datAreas[0];
		$nombreArea=$datAreas[1];
		
		$totalContactosMaestroVisitador=0;
		$totalContactosEjecutadoVisitador=0;
		$totalBajaContactosVisitador=0;
		
		$sqlVisitador="select distinct(f.codigo_funcionario), concat(f.paterno,' ',' ',f.nombres)as nombrefunc from rutero_maestro_cab_aprobado rc,
			funcionarios f where rc.cod_visitador=f.codigo_funcionario and rc.codigo_gestion=$rpt_gestion and rc.codigo_ciclo=$rpt_ciclo 
			and rc.estado_aprobado=1 and f.cod_ciudad in ($codArea) order by 2";
		$respVisitador=mysql_query($sqlVisitador);
		while($datVisitador=mysql_fetch_array($respVisitador)){
			$codVisitador=$datVisitador[0];
			$nombreVisitador=$datVisitador[1];
			...
			rc.cod_visitador in ($codVisitador) and rc.codigo_ciclo='$rpt_ciclo' and rc.estado_aprobado=1 and
			...
			$totalContactosMaestroVisitador=$totalContactosMaestroVisitador+$numero_contactos_maestro;
			...
			$sql_contactos_ejecutado="select count(*) from rutero_maestro_cab_aprobado rc, rutero_maestro_aprobado rm, rutero_maestro_detalle_aprobado rd
				...
				rm.cod_visitador=rd.cod_visitador and rc.codigo_gestion='$rpt_gestion' and rc.codigo_ciclo='$rpt_ciclo' 
				and rc.cod_visitador in ($codVisitador) 
			...
			$totalContactosEjecutadoVisitador=$totalContactosEjecutadoVisitador+$numero_contactos_ejecutado;
			
			
			$sqlNumVisitadores="SELECT count(*) from rutero_maestro_cab_aprobado rc 
			where rc.cod_visitador in ($codVisitador) 
			...
			$sqlBajasContactos="select count(*) from rutero_maestro_cab_aprobado rc, rutero_maestro_aprobado rm, rutero_maestro_detalle_aprobado rd
				...
				rm.cod_visitador=rd.cod_visitador and rc.codigo_gestion='$rpt_gestion' and rc.codigo_ciclo='$rpt_ciclo' 
				and rc.cod_visitador in ($codVisitador) 
				and rd.estado='2' and rd.categoria_med in ($rpt_categoria) and rm.dia_contacto like '%$ii%'";
			...
			$totalBajaContactosVisitador=$totalBajaContactosVisitador+$numeroBajas;
		...
		$totalNoRealizadoVisitador=$totalContactosMaestroVisitador-$totalContactosEjecutadoVisitador;
		$totalCoberturaVisitador=round(($totalContactosEjecutadoVisitador/($totalContactosMaestroVisitador-$totalBajaContactosVisitador))*100);
		$totalCumplimientoVisitador=round(($totalContactosEjecutadoVisitador/$totalContactosMaestroVisitador)*100);
		echo "<tr bgcolor='#FAAC58'><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><TH>Total $nombreArea</TH><th>$totalContactosMaestroVisitador</th><th>$totalContactosEjecutadoVisitador</th>
		<th>$totalNoRealizadoVisitador</th><th>$totalBajaContactosVisitador</th><th>-</th><th>$totalCumplimientoVisitador %</th><th>$totalCoberturaVisitador %</th></tr>";
	...
	echo "<tr bgcolor='#58FA58'><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><TH>Total $semanaX</TH><th>$totalContactosMaestroSemana</th><th>$totalContactosEjecutadoSemana</th>
	<th>$totalNoRealizadoSemana</th><th>$totalBajaContactosSemana</th><th>-</th><th>$totalCumplimientoSemana %</th><th>$totalCoberturaSemana %</th></tr>";
...
echo "<tr><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><TH>TOTALES</TH><th>$totalContactosMaestro</th><th>$totalContactosEjecutado</th>
<th>$totalNoRealizado</th><th>$totalBajaContactos</th><th>-</th><th>$totalCumplimiento %</th><th>$totalCobertura %</th></tr>";
