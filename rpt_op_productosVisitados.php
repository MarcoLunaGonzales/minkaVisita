...
{	var rpt_visitador,rpt_gestion, rpt_ciclo, rpt_ver;
	var rpt_territorio;
	...
	var rpt_dia=new Array();
	var rpt_nombreDia=new Array();
	...
	j=0;
	for(i=0;i<=f.rpt_dia.options.length-1;i++)
	{	if(f.rpt_dia.options[i].selected)
		{	rpt_dia[j]=f.rpt_dia.options[i].value;
			rpt_nombreDia[j]=f.rpt_dia.options[i].innerHTML;
			j++;
		}
	}
	window.open('rpt_productosVisitados.php?rpt_visitador='+rpt_visitador+'&rpt_gestion='+rpt_gestion+'&rpt_ciclo='+rpt_ciclo+'&rpt_territorio='+rpt_territorio+'&rpt_dia='+rpt_dia+'&rpt_nombreDia='+rpt_nombreDia+'&rpt_ver='+rpt_ver+'','','scrollbars=yes,status=no,toolbar=no,directories=no,menubar=no,resizable=yes,width=1000,height=800');
	return(true);
}
</script>
<?php
...
echo "<center><table class='textotit'><tr><th>Productos Visitados</th></tr></table><br>";
echo"<form method='post'>";
	...
	echo "<tr><th align='left'>Dia Contacto</th>
	<td><select name='rpt_dia' class='texto' size='20' multiple>";
	$sql="select o.id, o.`dia_contacto` from `orden_dias` o order by o.id";
	$resp=mysql_query($sql);
	while($dat=mysql_fetch_array($resp))
	{	$codDia=$dat[0];
		$nombreDia=$dat[1];
		echo "<option value='$codDia'>$nombreDia</option>";
	}
	echo "</select></td></tr>";
	
	echo "<tr><th align='left'>Ver</th>
	<td><select name='rpt_ver' class='texto'>
	<option value='2'>Resumido x Producto</option>
	<option value='0'>Resumido x Medico</option>
	<option value='1'>Detallado x Contacto</option></select>";
