...
//$indice_reporte    = 1;
$sqlXXX="select IFNULL(max(id_boleta+1),1) from boletas_visita_cab";
    ...
    while ($dat = mysql_fetch_array($resp)) {
        ...
        while ($dat1 = mysql_fetch_array($resp1)) {
			...
			$numFilasPersonalizada=1;
                ...
                $sql_insert_reporte_cab = "INSERT into boletas_visita_cab (id_boleta, id_visitador_hermes, id_visitador_zeus, territorio, visitador, medico, linea, 
				...
				while ($dat_parrilla_detalle = mysql_fetch_array($resp_parrilla_detalle)) {
					...
					$insert_reporte_det = "INSERT into boletas_visita_detalle (id_boleta, orden, muestra, cantidad_muestra, material, 
				...
				while($datPGE=mysql_fetch_array($respPGE)){
					...
					$insert_reporte_det = "INSERT into boletas_visita_detalle values($indice_reporte,$orden_promocional,'$muestra','$cant_muestra','$material','$cant_material',2)";
				...
				if($numero_visita==2){
					...
					while($datBancos=mysql_fetch_array($respBancos)){
						...
						$cant_material=0;
						$insert_reporte_det = "INSERT into boletas_visita_detalle values($indice_reporte,$orden_promocional,'$muestra','$cant_muestra','$material','',3)";
...
//verificar que maximo hayan 15 items.
	$sqlDelMax="update boletas_visita_detalle set material='--', cantidad_material='0' where material='Sin Material Apoyo'";
	$respDelMax=mysql_query($sqlDelMax);

	$sqlDelMax="delete from boletas_visita_detalle where orden_promocional>15";
	$respDelMax=mysql_query($sqlDelMax);
